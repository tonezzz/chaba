...
const GLAMA_MAX_TOKENS = Number.isFinite(Number(process.env.GLAMA_MAX_TOKENS))
  ...
  : 800;
const SYSTEM_PROMPT =
  (process.env.SYSTEM_PROMPT || 'You are an upbeat AI assistant for Surf Thailand. Keep responses concise and helpful.').trim();
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    return res.json({ reply: text, usage: data?.usage ?? null });
  } catch (error) {
    console.error('[site-chat] chat failed', error);
...
app.listen(PORT, () => {
  console.log(`site-chat listening on port ${PORT}`);
});
