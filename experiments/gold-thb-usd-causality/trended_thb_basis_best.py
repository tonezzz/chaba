"""Graph edge filter + k-NN combination with THB and XAG (silver) in the feature vector.

Run the same k-NN pattern matcher as trended_graph_knn.py, but the k-NN
feature vector uses THB (USD/THB) and silver (XAG/USD) returns. The graph edge is still
the THB -> XAU Granger-causality filter. Research only.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_thb_basis_best.py
"""
import json
import os
import sys
...
OVERSOLD = int(os.environ.get("OVERSOLD", "30"))
OVERBOUGHT = int(os.environ.get("OVERBOUGHT", "70"))
...
def load():
    if not (DATA / "aligned_silver.csv").exists():
        print("Run prepare_silver.py first to generate data/aligned_silver.csv", file=sys.stderr)
        raise SystemExit(1)
...
def main():
    ...
    for month in months:
        ...
        # RSI + ATR: block longs in overbought, shorts in oversold, size by ATR
        xau_rets = raw_train["xau_ret"]
        gains = xau_rets.clip(lower=0)
        losses = (-xau_rets).clip(lower=0)
        avg_gain = gains.tail(RSI_WINDOW).mean()
        avg_loss = losses.tail(RSI_WINDOW).mean()
        if avg_loss == 0:
            rsi = 100.0
        else:
            rsi = 100.0 - 100.0 / (1.0 + avg_gain / avg_loss)

        atr = xau_rets.tail(ATR_WINDOW).abs().mean()
        ...
        for d, yp, y in zip(month_dates, y_pred, y_test):
            pos = 0.0
            if yp > threshold and rsi < OVERBOUGHT:
                pos = leverage
            elif yp < -threshold and rsi > OVERSOLD:
                pos = -leverage
    ...
    print(f"Graph+k-NN THB+XAG+RSI+ATR: WINDOW={WINDOW}, K={K}, ATR_WINDOW={ATR_WINDOW}, TARGET_VOL={TARGET_VOL}, MAX_LEVERAGE={MAX_LEVERAGE}, RSI_WINDOW={RSI_WINDOW}, OVERSOLD={OVERSOLD}, OVERBOUGHT={OVERBOUGHT}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+RSI+ATR (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_thb_basis_best_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG returns plus RSI+ATR combined, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_thb_basis_best_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_thb_basis_best_equity.csv, data/trended_thb_basis_best_results.json")
