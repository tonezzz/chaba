...
feature vector uses THB (USD/THB) and silver (XAG/USD) returns. The graph gate requires both the THB -> XAU and the XAG -> XAU
Granger-causality edges to be significant. Research only.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_thb_xag_dual.py
"""
import json
import os
import sys
...
def granger_p(train, cause, maxlag=2):
    data = train[["xau_ret", cause]].dropna().values
...
def main():
    ...
    for month in months:
        ...
        p_thb = granger_p(raw_train, "thb_ret")
        p_xag = granger_p(raw_train, "xag_ret")
        p_value = min(p_thb, p_xag)
        edge_active = (p_thb <= P_THRESHOLD) and (p_xag <= P_THRESHOLD)
    ...
    print(f"Graph+k-NN THB+XAG dual: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG dual (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_thb_xag_dual_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG, gated by THB->XAU and XAG->XAU Granger edges. Research only.",
    }
    (DATA / "trended_graph_knn_thb_xag_dual_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_thb_xag_dual_equity.csv, data/trended_graph_knn_thb_xag_dual_results.json")
