"""Walk-forward delay-embedding k-NN (Takens-style) strategy.

Reconstruct a phase-space vector from delayed USD/THB log returns:
    [thb_t, thb_{t-1}, ..., thb_{t-WINDOW+1}]
For each day, find the K most similar historical delay vectors. Use their
future XAU returns as the forecast.

Research only, not a live trading strategy.

Usage:
    WINDOW=6 K=20 .venv/bin/python trended_delay.py
"""
import json
import os
import sys
...
def predict(X_train, y_train, X_query, k, weighted):
    nbrs = NearestNeighbors(n_neighbors=min(k + 1, len(X_train)), metric="euclidean")
    nbrs.fit(X_train)
    distances, indices = nbrs.kneighbors(X_query)
    y_pred = []
    for dists, idx in zip(distances, indices):
        # for query that is in train, drop self; otherwise the +1 gives an extra neighbor
        if len(dists) > k:
            dists = dists[:-1]
            idx = idx[:-1]
        if weighted:
...
def main():
    ...
    print(f"Delay-embedding k-NN: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Delay-embedding k-NN (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_delay_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "Takens-style delay-embedding k-NN on THB log returns. Research only.",
    }
    (DATA / "trended_delay_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_delay_equity.csv, data/trended_delay_results.json")
