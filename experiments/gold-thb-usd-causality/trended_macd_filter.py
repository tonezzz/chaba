...
Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_macd_filter.py
"""
import json
import os
import sys
...
def main():
    ...
    for month in months:
        ...
        # XAU MACD filter: allow longs when MACD > signal, shorts when MACD < signal
        xau_rets = raw_train["xau_ret"]
        ema_fast = xau_rets.ewm(span=MACD_FAST, adjust=False).mean()
        ema_slow = xau_rets.ewm(span=MACD_SLOW, adjust=False).mean()
        macd_line = ema_fast - ema_slow
        signal_line = macd_line.ewm(span=MACD_SIGNAL, adjust=False).mean()
        macd_val = macd_line.iloc[-1]
        signal_val = signal_line.iloc[-1]
        macd_bull = bool(macd_val > signal_val)

        for d, yp, y in zip(month_dates, y_pred, y_test):
            pos = 0.0
            if yp > threshold and macd_bull:
                pos = 1.0
            elif yp < -threshold and not macd_bull:
                pos = -1.0
    ...
    print(f"Graph+k-NN THB+XAG+MACD: WINDOW={WINDOW}, K={K}, MACD_FAST={MACD_FAST}, MACD_SLOW={MACD_SLOW}, MACD_SIGNAL={MACD_SIGNAL}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+MACD (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_macd_filter_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG returns plus XAU MACD filter, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_macd_filter_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_macd_filter_equity.csv, data/trended_macd_filter_results.json")
