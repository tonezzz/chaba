...
Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_ema_overlay.py
"""
import json
import os
import sys
...
SHORT = int(os.environ.get("SHORT", "5"))
LONG = int(os.environ.get("LONG", "20"))
...
def main():
    ...
    for month in months:
        ...
        # THB EMA trend overlay: only take longs in short>long, shorts in short<long
        thb_train = raw_train["thb_ret"]
        ema_short = thb_train.ewm(span=SHORT, adjust=False).mean().iloc[-1]
        ema_long = thb_train.ewm(span=LONG, adjust=False).mean().iloc[-1]
        ema_up = bool(ema_short > ema_long)

        for d, yp, y in zip(month_dates, y_pred, y_test):
            pos = 0.0
            if yp > threshold and ema_up:
                pos = 1.0
            elif yp < -threshold and not ema_up:
                pos = -1.0
    ...
    print(f"Graph+k-NN THB+XAG+EMA: WINDOW={WINDOW}, K={K}, SHORT={SHORT}, LONG={LONG}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+EMA (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_ema_overlay_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG returns plus THB EMA overlay, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_ema_overlay_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_ema_overlay_equity.csv, data/trended_ema_overlay_results.json")
