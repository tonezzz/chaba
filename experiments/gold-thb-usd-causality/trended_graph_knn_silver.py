"""Graph edge filter + k-NN combination with silver (XAG) in the feature vector.

Run the same k-NN pattern matcher as trended_graph_knn.py, but the k-NN
feature vector also includes XAG (silver) returns. The graph edge is still the
...
def main():
    ...
    print(f"Graph+k-NN+xag: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN+xag (walk-forward OOS) ===")
    ...
    summary = {
        ...
        "note": "k-NN pattern matcher with XAG returns, gated by THB->XAU Granger edge. Research only.",
    }
