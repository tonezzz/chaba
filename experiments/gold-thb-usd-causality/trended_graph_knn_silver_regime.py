"""Graph edge filter + k-NN with silver and volatility regime gate.

Run the same k-NN pattern matcher as trended_graph_knn_silver.py, but the
THB->XAU Granger gate is combined with a realized XAU volatility regime gate.
Trades are only allowed when the trailing XAU volatility is below a threshold.
Research only.

Usage:
    VOL_THRESHOLD=0.015 .venv/bin/python trended_graph_knn_silver_regime.py
"""
import json
import os
import sys
...
VOL_THRESHOLD = float(os.environ.get("VOL_THRESHOLD", "0.015"))
...
def main():
    ...
    for month in months:
        ...
        # Volatility regime: only trade when trailing XAU vol is below threshold
        train_rets = y_all[train_idx]
        regime_vol = float(np.nanstd(train_rets))
        edge_active = (p_value <= P_THRESHOLD) and (regime_vol <= VOL_THRESHOLD)
    ...
    print(f"Graph+k-NN+silver regime: WINDOW={WINDOW}, K={K}, VOL_TH={VOL_THRESHOLD}, P_THRESHOLD={P_THRESHOLD}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN+silver regime (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_silver_regime_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with silver and volatility regime gate. Research only.",
    }
    (DATA / "trended_graph_knn_silver_regime_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_silver_regime_equity.csv, data/trended_graph_knn_silver_regime_results.json")
