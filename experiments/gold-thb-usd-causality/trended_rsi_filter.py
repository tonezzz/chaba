...
Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_rsi_filter.py
"""
import json
import os
import sys
...
def main():
    ...
    for month in months:
        ...
        # XAU RSI filter: skip longs when overbought, shorts when oversold
    ...
    print(f"Graph+k-NN THB+XAG+RSI: WINDOW={WINDOW}, K={K}, RSI_WINDOW={RSI_WINDOW}, OVERSOLD={OVERSOLD}, OVERBOUGHT={OVERBOUGHT}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+RSI (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_rsi_filter_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG returns plus XAU RSI filter, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_rsi_filter_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_rsi_filter_equity.csv, data/trended_rsi_filter_results.json")
