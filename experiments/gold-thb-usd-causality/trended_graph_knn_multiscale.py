"""Graph edge filter + k-NN combination with multiscale history blocks in the feature vector.

Run the same k-NN pattern matcher as trended_graph_knn.py, but the k-NN
feature vector is built from M blocks of N consecutive daily returns (M * N total days). The graph edge is still
the THB -> XAU Granger-causality filter. Research only.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_multiscale.py
"""
import json
import os
import sys
...
def build_features(df, window):
    ...
    dates = []
    total = M * N
    for t in range(total - 1, n):
        block = rets[t - total + 1 : t + 1].reshape(M, N, len(cols))
        X.append(block.ravel().astype(np.float64))
...
def main():
    ...
    print(f"Graph+k-NN multiscale: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN multiscale (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_multiscale_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with multiscale (M x N) history blocks and silver, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_multiscale_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_multiscale_equity.csv, data/trended_graph_knn_multiscale_results.json")
