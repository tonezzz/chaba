"""Graph edge filter + k-NN with volatility-targeted position sizing.

Same as trended_graph_knn.py, but the position size is scaled by a target
annual volatility divided by the trailing realized XAU volatility. This keeps
the same THB->XAU edge gate but smooths exposure during high-volatility
regimes. Research only.

Usage:
    TARGET_VOL=0.10 MAX_LEVERAGE=2.0 .venv/bin/python trended_graph_knn_vol.py
"""
import json
import os
import sys
...
def main():
    ...
    print(f"Graph+k-NN vol-sized: WINDOW={WINDOW}, K={K}, TARGET_VOL={TARGET_VOL}, MAX_LEV={MAX_LEVERAGE}, P_THRESHOLD={P_THRESHOLD}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_vol_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with volatility sizing, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_vol_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_vol_equity.csv, data/trended_graph_knn_vol_results.json")
