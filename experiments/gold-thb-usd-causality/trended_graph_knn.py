"""Graph edge filter + k-NN combination.

Run the same k-NN pattern matcher as trended_knn.py, but only trade on days
where the THB -> XAU Granger-causality edge is significant. The graph edge
acts as a monthly regime gate on top of the k-NN signal.

Research only, not a live trading strategy.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn.py
"""
import json
import os
import sys
...
def main():
    ...
    print(f"Graph+k-NN: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    ...
    result.to_csv(DATA / "trended_graph_knn_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_equity.csv, data/trended_graph_knn_results.json")
