...
Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_sma_overlay.py
"""
import json
import os
import sys
...
def main():
    ...
    for month in months:
        ...
        # THB SMA trend overlay: only take longs in short>long, shorts in short<long
        thb_train = raw_train["thb_ret"]
        sma_short = thb_train.tail(SHORT).mean()
        sma_long = thb_train.tail(LONG).mean()
    ...
    print(f"Graph+k-NN THB+XAG+SMA: WINDOW={WINDOW}, K={K}, SHORT={SHORT}, LONG={LONG}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+SMA (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_sma_overlay_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAG returns plus THB SMA overlay, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_sma_overlay_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_sma_overlay_equity.csv, data/trended_sma_overlay_results.json")
