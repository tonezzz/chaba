"""Graph edge filter + k-NN combination for XAU/THB (THB settlement).

Run the same THB->XAU/USD Granger edge as the baseline, but the traded and
forecasted asset is XAU/THB. The k-NN features are USD/THB and XAU/USD
returns; the target is next-day XAU/THB return. Research only.

Usage:
    WINDOW=2 K=30 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_thb.py
"""
import json
import os
import sys
...
def main():
    ...
    print(f"Graph+k-NN+THB: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN+THB (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_thb_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher for XAU/THB, gated by USD/THB->XAU/USD Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_thb_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_thb_equity.csv, data/trended_graph_knn_thb_results.json")
