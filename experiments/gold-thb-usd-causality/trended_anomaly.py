"""Walk-forward k-NN with anomaly / change-point filter.

Same k-NN signal as trended_knn.py, but the position is forced to flat when
 the current THB/XAU window is farther from its k-th nearest training neighbor
than the ANOMALY_PCT percentile of training distances. This avoids trading in
unfamiliar regimes.

Research only, not a live trading strategy.

Usage:
    WINDOW=3 K=20 ANOMALY_PCT=90 .venv/bin/python trended_anomaly.py
"""
import json
import os
import sys
...
ANOMALY_PCT = float(os.environ.get("ANOMALY_PCT", "90"))
WEIGHTED = os.environ.get("WEIGHTED", "1") in ("1", "true", "True", "yes")
ANOMALY_PCT_GRID = [float(x) for x in os.environ.get("ANOMALY_PCT_GRID", "50,70,80,90,95,99").split(",")]
...
def search_anomaly_pct(y_pred, y_true, dist_k, threshold, pct_grid):
    best = {"sharpe": -np.inf, "pct": 100.0}
    for pct in pct_grid:
        if pct < 0 or pct > 100:
            continue
        th = np.percentile(dist_k, pct)
        signal = np.where((np.abs(y_pred) > threshold) & (dist_k <= th), np.sign(y_pred), 0.0)
        rets = signal * y_true
        if rets.std() == 0:
            continue
        sharpe = rets.mean() / rets.std() * np.sqrt(252)
        if sharpe > best["sharpe"]:
            best = {"sharpe": sharpe, "pct": float(pct)}
    return best["pct"]
...
def predict(X_train, y_train, X_query, k, weighted):
    ...
    for i, (dists, idx) in enumerate(zip(distances, indices)):
        ...
        dists = dists[mask][:k]
        idx = idx[mask][:k]
        dist_k.append(float(dists[-1]) if len(dists) else 0.0)
        if weighted:
...
def main():
    ...
    for month in months:
        ...
        train_pred, train_dist_k = predict(X_train_s, y_train, X_train_s, K, WEIGHTED)
        threshold = search_threshold(train_pred, y_train)
        anomaly_pct = search_anomaly_pct(train_pred, y_train, train_dist_k, threshold, ANOMALY_PCT_GRID)
        anomaly_threshold = np.percentile(train_dist_k, anomaly_pct)

        test_idx = np.where(month_mask)[0]
        X_test = X_all[test_idx]
        X_test_s = scaler.transform(X_test)
        y_test = y_all[test_idx]
        y_pred, dist_k = predict(X_train_s, y_train, X_test_s, K, WEIGHTED)

        for d, yp, y, dk in zip(month_dates, y_pred, y_test, dist_k):
            pos = 0.0
            if dk <= anomaly_threshold:
                if yp > threshold:
                    pos = 1.0
                elif yp < -threshold:
                    pos = -1.0
    ...
    print(f"Anomaly-k-NN: WINDOW={WINDOW}, K={K}, ANOMALY_PCT_GRID={','.join(str(int(p)) for p in ANOMALY_PCT_GRID)}, WEIGHTED={WEIGHTED}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== k-NN with anomaly filter (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_anomaly_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "anomaly_pct_grid": [float(p) for p in ANOMALY_PCT_GRID],
        "avg_monthly_anomaly_pct": float(np.mean([m["anomaly_pct"] for m in month_records])),
        "weighted": WEIGHTED,
        ...
        "note": "k-NN with anomaly flat-filter. Research only.",
    }
    (DATA / "trended_anomaly_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_anomaly_equity.csv, data/trended_anomaly_results.json")
