...
feature vector uses THB (USD/THB) and the XAU/XAG log-spread return. The graph edge is still
the THB -> XAU Granger-causality filter. Research only.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_thb_spread.py
"""
import json
import os
import sys
...
def main():
    ...
    print(f"Graph+k-NN THB+spread: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+spread (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_thb_spread_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with THB and XAU/XAG spread, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_thb_spread_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_thb_spread_equity.csv, data/trended_graph_knn_thb_spread_results.json")
