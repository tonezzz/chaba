  ...
  plot(
    ...
    [
      { x: x1, y: y1, mode: "lines", name: "Current THB window", line: { color: COLORS.thb } },
      {
        ...
        y: y2,
        mode: "lines",
        name: "Historical neighbor (warped)",
    ...
    [
      {
        ...
        mode: "lines",
        type: "scatter3d",
        name: "THB phase space",
        line: { color: COLORS.thb, width: 4 },
      },
    ],
    {
      ...
      scene: {
        xaxis: { title: "thb_t" },
        yaxis: { title: "thb_{t-1}" },
        zaxis: { title: "thb_{t-2}" },
      },
      height: 420,
    }
  );
...
function renderGraph() {
  ...
  for (const e of edges) {
    ...
    shapes.push({
      ...
      x1,
      y1,
      line: { color: "rgba(13,110,253," + e.w + ")", width: 2 + e.w * 4 },
    });
    shapes.push({
      type: "circle",
      x0: mx - 0.02,
    ...
    [
      {
        ...
        marker: { size: 20, color: COLORS.xau },
      },
    ],
    {
...
async function renderEquity() {
  ...
  try {
    ...
    if (!graph_knn) {
      container.innerHTML =
        '<p class="muted">Vector-analysis equity data not yet available. Run the backtests first.</p>';
      return;
    }
    ...
    if (trended) {
      traces.push({
        x: dates,
        y: trended.map((r) => r.equity),
        mode: "lines",
        name: "2-day THB trend",
        line: { color: "#0d6efd" },
      });
    }
    if (knn) {
    ...
    if (delay) {
      traces.push({
        x: dates,
        y: delay.map((r) => r.equity),
        mode: "lines",
        name: "Delay k-NN (W=6, K=20)",
        line: { color: "#6f42c1" },
      });
    }
    if (ssa) {
    ...
    if (rvar) {
      traces.push({
        x: dates,
        y: rvar.map((r) => r.equity),
        mode: "lines",
        name: "Regime VAR (l=2, vol=10)",
        line: { color: "#e83e8c" },
      });
    }
    if (graph) {
      traces.push({
        x: dates,
        y: graph.map((r) => r.equity),
        mode: "lines",
        name: "Graph edge filter",
        line: { color: "#6610f2" },
      });
    }
    if (wavelet) {
      traces.push({
        x: dates,
        y: wavelet.map((r) => r.equity),
        mode: "lines",
        name: "Wavelet (scales 4,8,16,32)",
        line: { color: "#0dcaf0" },
      });
    }
    if (graph_knn) {
    ...
    traces.push({
      x: dates,
      y: graph_knn_vol.map((r) => r.equity),
      mode: "lines",
      name: "Graph + k-NN vol-sized",
      line: { color: "#000000" },
    });
    if (graph_knn_silver) {
      traces.push({
        x: dates,
        y: graph_knn_silver.map((r) => r.equity),
        mode: "lines",
        name: "Graph + k-NN + XAG/silver",
        line: { color: "#C0C0C0" },
      });
    }
    if (graph_knn_vol_silver) {
      traces.push({
        x: dates,
        y: graph_knn_vol_silver.map((r) => r.equity),
        mode: "lines",
        name: "Graph + k-NN + XAG/silver vol-sized",
        line: { color: "#FFD700" },
      });
    }
