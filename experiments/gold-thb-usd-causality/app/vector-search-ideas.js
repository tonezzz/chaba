...
function renderWeighted() {
  ...
  const distances = [0.12, 0.18, 0.25, 0.4, 0.55];
...
async function renderMDDB() {
  ...
  try {
    const results = await fetch("data/mddb_results.json")
      .then((r) => r.json())
      .catch(() => null);
    if (!results) {
...
async function renderEquity() {
  ...
  try {
    ...
    if (!trended || !knn || !weighted || !anomaly) {
      container.innerHTML =
        '<p class="muted">Equity data not yet available on this host. Run the backtests or load from the repo.</p>';
      return;
    }
    ...
    const metrics = [
      ...
      { name: "Kernel weighted (γ=0.5)", s: 1.23, r: 0.16, dd: -0.508, w: 0.303 },
