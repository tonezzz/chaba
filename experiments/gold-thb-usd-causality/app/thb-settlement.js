...
async function renderEquity() {
  ...
  try {
    ...
    if (!xau_usd || !xau_thb || !hold_usd) {
      container.innerHTML =
        '<p class="muted">Equity data not yet available. Run trended_graph_knn_silver.py, trended_graph_knn_thb.py, and trended_graph_knn_hold_usd.py first.</p>';
      return;
    }
    ...
    const traces = [
      {
        x: dates,
        y: xau_usd.map((r) => r.equity),
        mode: "lines",
        name: "XAU/USD silver (ref)",
        line: { color: "#C0C0C0" },
      },
      {
        x: dates,
        y: hold_usd.map((r) => r.equity),
        mode: "lines",
        name: "XAU/USD position, USD marked to THB",
        line: { color: "#0d6efd" },
      },
      {
        x: dates,
        y: xau_thb.map((r) => r.equity),
        mode: "lines",
        name: "XAU/THB position (settle THB)",
        line: { color: "#FFD700" },
      },
    ];
