...
function renderReturns(rets) {
  ...
  const data = [
    trace(
      "XAU return",
      dates,
      rets.map((r) => r.xau_ret),
      "#d4af37"
    ),
    trace(
      "THB return",
      dates,
      rets.map((r) => r.thb_ret),
      "#0d6efd"
    ),
    trace(
      "USD return",
      dates,
      rets.map((r) => r.usd_ret),
      "#dc3545"
    ),
  ];
  plot("returns", data, {
...
function renderStrategyEquity(equity) {
  if (!equity || equity.length === 0) {
    document.getElementById("strategy-equity").innerHTML =
      '<p class="muted">No equity data. Run <code>trended.py</code> or <code>walkforward.py</code> first.</p>';
    return;
  }
  ...
  const data = [
    trace(
      "Strategy equity",
      dates,
      equity.map((r) => r.strategy_equity),
      "#0d6efd"
    ),
    trace(
      "Buy-and-hold XAU",
      dates,
      equity.map((r) => r.buyhold_equity),
      "#d4af37"
    ),
  ];
...
function renderCrossCorr(results) {
  if (!results || !results.cross_correlation_peak || !results.cross_correlation_peak.all_lags) {
    document.getElementById("cross-corr").innerHTML =
      '<p class="muted">No cross-correlation data. Run <code>analyze.py</code>.</p>';
    return;
  }
  const lags = Object.keys(results.cross_correlation_peak.all_lags)
    .map(Number)
    .sort((a, b) => a - b);
  const corrs = lags.map((l) => results.cross_correlation_peak.all_lags[l.toString()]);
  const colors = corrs.map((v) => (v >= 0 ? "#198754" : "#dc3545"));
  plot("cross-corr", [{ x: lags.map(String), y: corrs, type: "bar", marker: { color: colors } }], {
...
function renderIRF(results) {
  if (!results || !results.impulse_response_xau) {
    document.getElementById("irf").innerHTML =
      '<p class="muted">No impulse-response data. Run <code>analyze.py</code>.</p>';
    return;
  }
...
function populateText(results, aligned) {
  if (results) {
    ...
    document.getElementById("range-dates").textContent =
      `${aligned[0].date} → ${aligned[aligned.length - 1].date}`;
  }
}
...
async function init() {
  ...
  try {
    let equity = await fetchCSV("/data/trended_equity.csv").catch(
      async () => await fetchCSV("/data/walkforward_equity.csv").catch(() => null)
    );
    ...
    if (!aligned && !rets) {
      status.innerHTML =
        '<span class="error">No data available. Run <code>analyze.py</code> first to generate <code>data/aligned.csv</code> and <code>data/returns.csv</code>.</span>';
      return;
    }
