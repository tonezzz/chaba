"""Graph edge filter + k-NN with an alternative distance metric.

Run the same k-NN pattern matcher as trended_graph_knn.py, but allow the
neighbor distance to be correlation, cosine, manhattan, or euclidean. Research
only.

Usage:
    METRIC=correlation .venv/bin/python trended_graph_knn_dist.py
"""
import json
import os
import sys
...
METRIC = os.environ.get("METRIC", "correlation")
...
def main():
    ...
    scale = METRIC in ("euclidean", "manhattan", "minkowski")
    ...
    for month in months:
        ...
        if scale:
            ...
            X_test = X_all[np.where(month_mask)[0]]
            X_test_s = scaler.transform(X_test)
        else:
            X_train_s = X_train
            X_test_s = X_all[np.where(month_mask)[0]]

        knn = NearestNeighbors(n_neighbors=min(K + 1, len(X_train_s)), metric=METRIC)
        knn.fit(X_train_s)
    ...
    print(f"Graph+k-NN ({METRIC}): WINDOW={WINDOW}, K={K}, P_THRESHOLD={P_THRESHOLD}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print(f"=== Graph+k-NN {METRIC} (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / f"trended_graph_knn_{METRIC}_equity.csv")
    summary = {
        "metric": METRIC,
        "window": WINDOW,
        "k": K,
        ...
        "note": f"k-NN with {METRIC} distance. Research only.",
    }
    (DATA / f"trended_graph_knn_{METRIC}_results.json").write_text(json.dumps(summary, indent=2))
    print(f"Saved: data/trended_graph_knn_{METRIC}_equity.csv, data/trended_graph_knn_{METRIC}_results.json")
