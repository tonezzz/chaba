"""Graph edge filter + k-NN on THB-base (XAU/THB) with volatility targeting.

The target is the next-day log return of XAU/THB. The k-NN feature vector
uses USD/THB and XAU/THB returns. The graph edge is THB/USD -> XAU/THB
Granger-causality. Research only.

Usage:
    WINDOW=2 K=30 TARGET_VOL=0.20 .venv/bin/python trended_graph_knn_vol_thb_base.py
"""
import json
import os
import sys
...
def load():
    ...
    # rename for consistency with the original graph+k-NN pipeline
    df = df.rename(columns={"xau_thb": "xau", "usd_thb": "thb"})
    logdf = np.log(df)
    rets = logdf.diff().dropna()
    rets.columns = ["xau_usd_ret", "thb_ret", "xau_ret"]
...
def main():
    ...
    print(f"THB-base vol-sized: WINDOW={WINDOW}, K={K}, TARGET_VOL={TARGET_VOL}, MAX_LEV={MAX_LEVERAGE}, P_THRESHOLD={P_THRESHOLD}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== THB-base graph+k-NN vol-sized (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_vol_thb_base_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "THB-base (XAU/THB) k-NN with volatility sizing. Research only.",
    }
    (DATA / "trended_graph_knn_vol_thb_base_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_vol_thb_base_equity.csv, data/trended_graph_knn_vol_thb_base_results.json")
