...
Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_atr_sizing.py
"""
import json
import os
import sys
...
def main():
    ...
    for month in months:
        ...
        # ATR-based position sizing: scale the k-NN signal by target vol / ATR
    ...
    print(f"Graph+k-NN THB+XAG+ATR: WINDOW={WINDOW}, K={K}, ATR_WINDOW={ATR_WINDOW}, TARGET_VOL={TARGET_VOL}, MAX_LEVERAGE={MAX_LEVERAGE}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN THB+XAG+ATR (walk-forward OOS) ===")
