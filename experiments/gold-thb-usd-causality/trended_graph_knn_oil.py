"""Graph edge filter + k-NN combination with oil in the feature vector.

Run the same k-NN pattern matcher as trended_graph_knn.py, but the k-NN
feature vector also includes oil (WTI) returns. The graph edge is still the
THB -> XAU Granger-causality filter. Research only.

Usage:
    WINDOW=3 K=20 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_oil.py
"""
import json
import os
import sys
...
def load():
    ...
    rets.columns = ["xau_ret", "thb_ret", "oil_ret", "brent_ret"]
...
def main():
    ...
    print(f"Graph+k-NN+oil: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN+oil (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_oil_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "k-NN pattern matcher with oil returns, gated by THB->XAU Granger edge. Research only.",
    }
    (DATA / "trended_graph_knn_oil_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_oil_equity.csv, data/trended_graph_knn_oil_results.json")
