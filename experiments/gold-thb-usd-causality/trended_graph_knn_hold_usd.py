"""Graph edge filter + k-NN: hold USD, account in THB.

Same THB->XAU/USD graph edge and k-NN as the baseline, but the gold position is
in XAU/USD and the USD cash is marked to THB each day. The P/L is therefore
(pos * xau_usd_next) + (usd_thb_next) minus transaction cost. Research only.

Usage:
    WINDOW=2 K=30 LOOKBACK=252 P_THRESHOLD=0.01 .venv/bin/python trended_graph_knn_hold_usd.py
"""
import json
import os
import sys
...
def load():
    if not (DATA / "aligned_thb_base.csv").exists():
        print("Run prepare_thb_base.py first to generate data/aligned_thb_base.csv", file=sys.stderr)
        raise SystemExit(1)
    ...
    rets["xau_t1"] = logdf["xau_usd"].shift(-1) - logdf["xau_usd"]
    return rets.dropna()


def build_features(df, window):
    cols = ["usd_thb_ret", "xau_usd_ret"]
    rets = df[cols].values
    n = len(rets)
    X = []
    dates = []
...
def granger_p(train, maxlag=2):
    data = train[["xau_usd_ret", "usd_thb_ret"]].dropna().values
...
def main():
    ...
    for month in months:
        ...
        if len(train_idx) < max(WINDOW + K, 63) or not edge_active:
            for d in month_dates:
                thb = df.loc[d, "usd_thb_ret"]
                xau_thb_t1 = df.loc[d, "xau_t1"] + thb
                records.append({
                    "date": d,
                    "position": 0.0,
                    "y_pred": 0.0,
                    "p_value": float(p_value),
                    "xau_t1": float(xau_thb_t1),
                    "strategy_ret": float(thb),
                    "equity": equity,
                })
        ...
        for d, yp, y in zip(month_dates, y_pred, y_test):
            ...
            if yp > threshold:
                ...
                pos = -1.0
            thb = df.loc[d, "usd_thb_ret"]
            ret = pos * y + thb - TC * abs(pos - prev_pos)
            prev_pos = pos
            equity *= np.exp(ret)
            xau_thb_t1 = y + thb
            records.append({
    ...
    print(f"Graph+k-NN+hold-USD: WINDOW={WINDOW}, K={K}, WEIGHTED={WEIGHTED}, P_THRESHOLD={P_THRESHOLD}, MAX_LAG={MAX_LAG}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== Graph+k-NN+hold-USD (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_graph_knn_hold_usd_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "weighted": WEIGHTED,
        ...
        "note": "XAU/USD graph+k-NN with USD cash marked to THB. Research only.",
    }
    (DATA / "trended_graph_knn_hold_usd_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_graph_knn_hold_usd_equity.csv, data/trended_graph_knn_hold_usd_results.json")
