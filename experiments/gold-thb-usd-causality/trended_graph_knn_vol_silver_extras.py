"""Graph edge filter + k-NN with silver and an extra asset, plus vol targeting.

Same as trended_graph_knn_vol_silver.py, but the feature vector also adds one
extra asset (oil, brent, or usd). The graph edge is still the THB -> XAU
Granger-causality filter. Research only.

Usage:
    EXTRA_COL=oil .venv/bin/python trended_graph_knn_vol_silver_extras.py
"""
import json
import os
import sys
...
EXTRA_COL = os.environ.get("EXTRA_COL", "oil")


def load():
    if not (DATA / "aligned_silver_multi.csv").exists():
        print("Run prepare_silver_multi.py first to generate data/aligned_silver_multi.csv", file=sys.stderr)
        raise SystemExit(1)
    df = pd.read_csv(DATA / "aligned_silver_multi.csv", parse_dates=["date"])
    df = df.sort_values("date").dropna()
    df.set_index("date", inplace=True)
    logdf = np.log(df)
    rets = logdf.diff().dropna()
    # rename columns for clarity
    ret_cols = {c: f"{c}_ret" for c in df.columns}
    rets = rets.rename(columns=ret_cols)
    rets["xau_t1"] = logdf["xau"].shift(-1) - logdf["xau"]
    return rets.dropna()


def build_features(df, window, extra):
    cols = ["thb_ret", "xau_ret", "xag_ret", f"{extra}_ret"]
    rets = df[cols].values
    n = len(rets)
    X = []
    dates = []
...
def main():
    df = load()
    X_all, dates = build_features(df, WINDOW, EXTRA_COL)
    ...
    print(f"Graph+k-NN+silver+{EXTRA_COL} vol-sized: WINDOW={WINDOW}, K={K}, TARGET_VOL={TARGET_VOL}, MAX_LEV={MAX_LEVERAGE}, P_THRESHOLD={P_THRESHOLD}, EDGE_LB={EDGE_LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print(f"=== Graph+k-NN+silver+{EXTRA_COL} vol-sized (walk-forward OOS) ===")
    ...
    suffix = EXTRA_COL.replace("/", "_")
    result.to_csv(DATA / f"trended_graph_knn_vol_silver_{suffix}_equity.csv")
    summary = {
        "window": WINDOW,
        "k": K,
        "extra_col": EXTRA_COL,
        ...
        "note": f"k-NN with THB, XAU, XAG, and {EXTRA_COL} returns, with volatility sizing. Research only.",
    }
    (DATA / f"trended_graph_knn_vol_silver_{suffix}_results.json").write_text(json.dumps(summary, indent=2))
    print(f"Saved: data/trended_graph_knn_vol_silver_{suffix}_equity.csv, data/trended_graph_knn_vol_silver_{suffix}_results.json")
