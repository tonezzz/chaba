"""Walk-forward 2-day USD/EUR (WTI) trend baseline for XAU.
...
def load():
    if not (DATA / "aligned_usd.csv").exists():
        print("Run prepare_oil.py first to generate data/aligned_usd.csv", file=sys.stderr)
        raise SystemExit(1)
    df = pd.read_csv(DATA / "aligned_usd.csv", parse_dates=["date"])
    df = df.sort_values("date").dropna()
    df.set_index("date", inplace=True)
    logdf = np.log(df)
    col = os.environ.get("USD_COL", "usd")
...
def main():
    ...
    print(f"USD/EUR trend: TREND_WINDOW={TREND_WINDOW}, LOOKBACK={LOOKBACK}, TC={TC*100:.4f}%")
    print(f"Total OOS days: {len(result)}")
    print(f"Date range: {result.index.min().date()} to {result.index.max().date()}")
    print()

    print("=== 2-day USD/EUR trend (walk-forward OOS) ===")
    ...
    result.to_csv(DATA / "trended_usd_equity.csv")
    summary = {
        ...
        "note": "2-day USD/EUR trend signal for XAU. Research only.",
    }
    (DATA / "trended_usd_results.json").write_text(json.dumps(summary, indent=2))
    print("Saved: data/trended_usd_equity.csv, data/trended_usd_results.json")
