"""Grid search for graph edge + k-NN combination.

Search over WINDOW, K, P_THRESHOLD and EDGE_LOOKBACK, reusing the same
k-NN neighbor query for all K values at a given window. Results are written
to data/graph_knn_silver_tune_grid.csv and data/graph_knn_silver_tune_results.json.
...
FEATURE_SETS = [x.strip() for x in os.environ.get("FEATURE_SETS", "thb_xau_xag,thb_xau,thb_xag,xau_xag,xag").split(",")]
KS = [int(x) for x in os.environ.get("KS", "10,20,30,50").split(",")]
P_THRESHOLDS = [float(x) for x in os.environ.get("P_THRESHOLDS", "0.01,0.05").split(",")]
EDGE_LOOKBACKS = [int(x) for x in os.environ.get("EDGE_LOOKBACKS", "252,504").split(",")]
...
def build_features(df, window, features):
    rets = df[features].values
    X = []
    dates = []
...
def main():
    ...
    # Precompute features for each feature set
    feature_data = {}
    all_dates = df.index
    for fs in FEATURE_SETS:
        features = [f"{c}_ret" for c in fs.split("_")]
        X, fdates = build_features(df, WINDOW, features)
        y = df.loc[fdates, "xau_t1"].values
        feature_data[fs] = {"X": X, "dates": fdates, "y": y, "features": features}

    combos = list(product(FEATURE_SETS, KS, P_THRESHOLDS, EDGE_LOOKBACKS))
    ...
    results.to_csv(DATA / "graph_knn_tune_grid.csv", index=False)
    (DATA / "graph_knn_tune_results.json").write_text(json.dumps(records, indent=2))
    print("Saved: data/graph_knn_silver_tune_grid.csv, data/graph_knn_silver_tune_results.json")
