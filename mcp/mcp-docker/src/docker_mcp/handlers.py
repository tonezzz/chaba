...
class DockerHandlers:
    ...
    @staticmethod
    async def handle_compose_control(arguments: Dict[str, Any]) -> List[TextContent]:
        debug_info: List[str] = []
        compose_path = arguments.get("compose_path")
        project_name = arguments.get("project_name")
        command = arguments.get("command")
        flags = arguments.get("flags") or []
        services = arguments.get("services") or []

        if not compose_path or not project_name or not command:
            raise ValueError("compose_path, project_name, and command are required")

        compose_path = os.path.abspath(compose_path)
        debug_info.append(f"Using compose file: {compose_path}")
        ...
        try:
            code, out, err = await compose.run(command, flags, services)
            debug_info.extend([
                f"Command: docker compose -f {compose_path} -p {project_name} {command} {' '.join(flags + services)}",
                f"Return Code: {code}",
                f"Stdout:\n{out}",
                f"Stderr:\n{err}"
            ])
            ...
            return [TextContent(
                type="text",
                text=f"Compose command {status} with exit code {code}\n\nDebug Info:\n{chr(10).join(debug_info)}"
            )]
        except Exception as e:
            debug_info.append(f"Error: {str(e)}")
            return [TextContent(type="text", text=f"Error running compose command: {str(e)}\n\nDebug Info:\n{chr(10).join(debug_info)}")]
