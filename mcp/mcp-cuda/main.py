...
# Preview decode tuning: decoding full-resolution latents is expensive and often looks like noise.
# We downsample latents before VAE decode to reduce CPU/GPU load.
MCP_CUDA_PREVIEW_DECODE_MAX_SIZE = int(os.getenv("MCP_CUDA_PREVIEW_DECODE_MAX_SIZE", "256"))
...
def _latents_to_preview_png_base64(pipe: StableDiffusionXLPipeline, latents: Any) -> Optional[str]:
    ...
    try:
        with torch.no_grad():
            ...
            # Downsample before decode to reduce overhead.
            # Latents are in (B, C, H, W) where H/W are image/8.
            try:
                max_px = int(MCP_CUDA_PREVIEW_DECODE_MAX_SIZE or 0)
                if max_px > 0 and lat_f32.ndim == 4:
                    _, _, lh, lw = lat_f32.shape
                    target = max(8, int(max_px) // 8)
                    if lh > target or lw > target:
                        lat_f32 = torch.nn.functional.interpolate(
                            lat_f32,
                            size=(min(lh, target), min(lw, target)),
                            mode="bilinear",
                            align_corners=False,
                        )
            except Exception:
            ...
            if pil is None:
                ...
                # Avoid autocast-induced weirdness during VAE decode.
                autocast_ctx = (
                ...
                try:
                    if bool(torch.isnan(image).any()) or bool(torch.isinf(image).any()):
                        return None
                except Exception:
                    pass
