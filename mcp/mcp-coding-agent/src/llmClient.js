...
export const callChatCompletions = async ({ baseUrl, apiKey, model, messages, temperature = 0 }) => {
  ...
  if (!url) {
    throw new Error('Missing LLM endpoint (set MCP_CODING_AGENT_LLM_BASE_URL or GLAMA_API_URL)');
  }
  if (!model) {
    throw new Error('Missing model (set MCP_CODING_AGENT_LLM_MODEL or GLAMA_MODEL)');
  }
