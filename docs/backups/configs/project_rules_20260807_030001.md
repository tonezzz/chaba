...
- Use ssot-search skill for searching SSOT YAML files (exact pattern matching) or all documentation (MCP docs server)
- For SSOT-specific searches: Use ssot-search with "SSOT YAML only" option for exact YAML pattern matching
- For broad documentation searches: Use ssot-search with "all documentation (MCP docs server)" option for full-text search with relevance ranking
- MCP docs server (@devista/docs-mcp) provides: search_docs, get_page, list_sections tools for 58+ documentation files
- See `docs/kb/documentation-search.md` for comprehensive guide on dual search methods
- MCP docs server auto-indexes docs/ directory with Flexsearch, cached in .docs-mcp/
