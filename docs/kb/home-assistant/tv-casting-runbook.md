...
| Plug auto-power gates | `script.cast_power_on` + `wakeCastTarget` (cast-browser-server.mjs) | both check `cast_enabled` before touching `switch.plug_tv`; **10-min manual-off cooldown** — if `plug_tv` transitioned to `off`/`unavailable` <10 min ago, auto power-on is suppressed + notified; plug `turn_on` only fires when the plug isn't already `on`; every real auto-power emits a persistent notification (`cast_plug_autopower`, `cast_plug_cooldown`, `cast_power_on_disabled`) |
...
| **Plug turned off while cast paths active** | any `off` transition starts a 10-min auto-power cooldown (no helper needed — `cast_power_on`/`wakeCastTarget` compare `plug_tv.last_changed`); re-power attempts suppressed + notified; after 10 min normal recovery resumes. Caveat: `last_changed` resets at HA start, so a restart while the plug is `off` effectively restarts the cooldown | power the plug yourself, or wait out the cooldown |
