...
function parseArgs() {
  ...
  const session =
    sIdx >= 0 ? args[sIdx + 1] : process.env.DEVIN_SESSION_ID || process.pid.toString();
...
function classify(query) {
  ...
  if (
    /(\b|^)(service|down|restart|status|health|container|podman|docker|systemctl|logs)(\b|$)/.test(
      q
    )
  )
    return "service-health";
  if (
    /(\b|^)(ssot|config\/ssot|docs\/ssot|focus|jobs|infrastructure|services|terminology|health|improvements|playlive|preset)(\b|$)/.test(
      q
    )
  )
    return "ssot";
  if (
    /(\b|^)(file|function|class|script|code|where|grep|search.*code|implementation|bug)(\b|$)/.test(
      q
    )
  )
    return "code";
  if (
    /(\b|^)(stock|swap|restructuring|corporate action|dividend|split|amalgamation|banpu|settrade|ptt|price jump|jump|news|current event|google|web search|web|external)(\b|$)/.test(
      q
    )
  )
    return "web";
...
function recordResult({ session, result }) {
  ...
  fs.appendFileSync(
    ...
    JSON.stringify({ t: new Date().toISOString(), result }) + "\n"
  );
}
...
function main() {
  ...
  if (!query) {
    console.error(
      'Usage: info-find.mjs --query "<query>" [--session <id>] [--quick] [--web] [--max-results N] [--record-result <json>]'
    );
