if (typeof GMP_DATA == 'undefined') var GMP_DATA = {};
if (isNumber(GMP_DATA.animationSpeed)) GMP_DATA.animationSpeed = parseInt(GMP_DATA.animationSpeed);
else if (jQuery.inArray(GMP_DATA.animationSpeed, ['fast', 'slow']) == -1) GMP_DATA.animationSpeed = 'fast';
...
jQuery.fn.sendFormGmp = function (params) {
  ...
  if (msgEl && !params.btn) {
    jQuery(msgEl).removeClass('gmpSuccessMsg').removeClass('gmpErrorMsg').showLoaderGmp();
  }
  if (params.btn) {
  ...
  else if (typeof ajaxurl == 'undefined') url = GMP_DATA.ajaxurl;
  else url = ajaxurl;
...
function toeProcessAjaxResponseGmp(res, msgEl, form, sentFromForm, params) {
  ...
  if (typeof res == 'object') {
    if (res.error) {
      if (msgEl) {
        jQuery(msgEl).removeClass('gmpSuccessMsg').addClass('gmpErrorMsg');
      }
      var errorsArr = [];
      for (var name in res.errors) {
        ...
        if (jQuery('.gmpErrorForField.toe_' + nameToClassId(name) + '').exists())
          jQuery('.gmpErrorForField.toe_' + nameToClassId(name) + '')
            .show()
      ...
      if (msgEl) {
        jQuery(msgEl).removeClass('gmpErrorMsg').addClass('gmpSuccessMsg');
...
function toeOptionGmp(key) {
  if (GMP_DATA.options && GMP_DATA.options[key] && GMP_DATA.options[key].value) return GMP_DATA.options[key].value;
  return false;
}
function toeLangGmp(key) {
  if (GMP_DATA.siteLang && GMP_DATA.siteLang[key]) return GMP_DATA.siteLang[key];
  return key;
}
function toePagesGmp(key) {
  if (typeof GMP_DATA != 'undefined' && GMP_DATA[key]) return GMP_DATA[key];
  return false;
}
/**
...
function toeOptTimeoutHideDescriptionGmp() {
  ...
  setTimeout(function () {
    if (!jQuery('#gmpOptDescription').attr('toeFixTip')) toeOptHideDescriptionGmp();
  }, 500);
}
/**
...
function toeOptShowDescriptionGmp(description, x, y, moveToLeft) {
  if (typeof description != 'undefined' && description != '') {
    ...
    else jQuery('#gmpOptDescription').css('left', x + 10);
