...
gmpGoogleMap.prototype._fixZoomLevel = function () {
  ...
  if (!eventHandle) {
    eventHandle = google.maps.event.addListener(
      this.getRawMapInstance(),
      'zoom_changed',
      jQuery.proxy(function () {
        ...
        if (this.getZoom() < minZoom) {
          ...
          if (GMP_DATA.isAdmin && this._getEventListenerHandle('idle', 'enableClasterization')) google.maps.event.trigger(this.getRawMapInstance(), 'idle');
        }
...
gmpGoogleMap.prototype.customClastererCalculatorFunction = function (markerGroupsStyles) {
  return function (markers, numStyles) {
    ...
    if (curStyle && curStyle[0]) styleIndex = markersGroupsStyles.indexOf(curStyle[0]) + 1;
...
gmpGoogleMap.prototype._getEventListenerHandle = function (event, code) {
  return this._eventListeners[event] && this._eventListeners[event][code] ? this._eventListeners[event][code] : false;
};
...
gmpGoogleMap.prototype.setCenter = function (lat, lng) {
  ...
  } else this.getRawMapInstance().setCenter(new google.maps.LatLng(lat, lng));
  return this;
};
  ...
  jQuery(this).keyup(function (event) {
    ...
    if (searchData && searchData != '') {
      ...
      jQuery(this).autocomplete(
        jQuery.extend({}, params.autocompleteParams, {
