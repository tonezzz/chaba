...
function pointAlong(pts, fraction = 0.5) {
  if (!Array.isArray(pts) || pts.length < 2) return { point: pts ? pts[0] : null, bearing: 0 };
  if (pts.length === 2) {
    return { point: midpoint(pts[0], pts[1]), bearing: bearing(pts[0], pts[1]) };
  }
  const dists = [];
  let total = 0;
  ...
  return { point: pts[pts.length - 1], bearing: 0 };
}
...
const Course = {
  ...
  roundBuoyArc, pointSide, resolveSide, pointAlong
};
