...
| Shared-path rebuild rules for cross-context dependencies | Done | `mcp-bundle` rebuild triggers on `services/assistance/mcp-servers/mcp-google-sheets/server.js` |
...
| Operator smoke checklist: calendar reminder + legacy scheduler disabled | Done | Added section in `ACTION.md`: "Operator smoke checklist (Calendar + legacy scheduler)" |
...
| Calendar cutover | Add explicit operator endpoint/tool for listing recent Calendar-created reminders | Makes smoke test fully backend-driven without opening Google Calendar |
