...
function buildWhereClause(filters) {
  ...
  if (filters.chatName) {
    operands.push(
      `{path:["chatName"] operator:Like valueText:${JSON.stringify("*" + filters.chatName + "*")}}`
    );
  }
  if (filters.fromName) {
    operands.push(
      `{path:["fromName"] operator:Like valueText:${JSON.stringify("*" + filters.fromName + "*")}}`
    );
  }
  if (filters.category) {
    operands.push(
  ...
  if (filters.sinceTime) {
    operands.push(
      `{path:["deliveredTime"] operator:GreaterThanEqual valueInt:${filters.sinceTime}}`
    );
  }
