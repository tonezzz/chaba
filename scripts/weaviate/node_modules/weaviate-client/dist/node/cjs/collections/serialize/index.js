...
class Search {
    ...
    static metadata = (includeVector, metadata) => {
        ...
        if (!MetadataGuards.isKeys(metadata)) {
            throw new errors_js_1.WeaviateInvalidInputError(`Invalid returnMetadata argument: ${metadata}.`);
        }
    ...
    static selection = (diversity) => {
        if (index_js_3.DiversityGuards.isMMR(diversity)) {
            return {
                mmr: base_search_js_1.Selection_MMR.fromPartial({
...
class Serialize {
    ...
    static boostCurve(curve) {
        switch (curve) {
            case 'exponential':
                return search_get_js_1.Boost_DecayCurve.DECAY_CURVE_EXPONENTIAL;
            case 'gaussian':
                return search_get_js_1.Boost_DecayCurve.DECAY_CURVE_GAUSS;
            case 'linear':
                return search_get_js_1.Boost_DecayCurve.DECAY_CURVE_LINEAR;
            default:
                return search_get_js_1.Boost_DecayCurve.DECAY_CURVE_UNSPECIFIED;
        }
    }
    static boostModifier(modifier) {
        switch (modifier) {
            case 'log1p':
                return search_get_js_1.Boost_PropertyValueModifier.PROPERTY_VALUE_MODIFIER_LOG1P;
            case 'sqrt':
                return search_get_js_1.Boost_PropertyValueModifier.PROPERTY_VALUE_MODIFIER_LOG1P;
            default:
                return search_get_js_1.Boost_PropertyValueModifier.PROPERTY_VALUE_MODIFIER_UNSPECIFIED;
        }
    }
    static boostGRPC = (boost) => {
        const out = { weight: boost.weight, depth: boost.depth, conditions: [] };
        for (const cond of boost.conditions) {
            let condProto = { weight: cond.weight };
            const { type, ...func } = cond.func;
            switch (type) {
                case 'filter':
                    condProto = {
                        ...condProto,
                        filter: this.filtersGRPC(func),
                    };
                    break;
                case 'timeDecay': {
                    const { curve, ...timeDecay } = func;
                    condProto = {
                        ...condProto,
                        timeDecay: search_get_js_1.Boost_TimeDecayFunction.fromPartial({
                            ...timeDecay,
                ...
                case 'numericDecay': {
                    const { curve, ...numericDecay } = func;
                    condProto = {
                        ...condProto,
                        numericDecay: search_get_js_1.Boost_NumericDecayFunction.fromPartial({
                            ...numericDecay,
                            curve: this.boostCurve(curve),
                        }),
                    };
                ...
                case 'propertyValue': {
                    const { modifier, ...propertyValue } = func;
                    condProto = {
                        ...condProto,
                        propertyValue: search_get_js_1.Boost_PropertyValueFunction.fromPartial({
                            ...propertyValue,
