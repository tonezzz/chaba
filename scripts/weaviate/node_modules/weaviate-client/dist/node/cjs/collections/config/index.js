...
const config = (connection, name, dbVersionSupport, tenant) => {
    ...
    return {
        ...
        addReference: (reference) => new index_js_1.PropertyCreator(connection)
            ...
            .do()
            .then(() => { }),
        addVector: async (vectors) => {
            const { vectorsConfig } = (0, utils_js_1.makeVectorsConfig)(vectors);
            const { supports: serverAppliesDefaultVectorIndexType } = await dbVersionSupport.supportsServerSideDefaultVectorIndexType();
        ...
        dropInvertedIndex: (propertyName, indexName) => connection.delete(`/schema/${name}/properties/${propertyName}/index/${indexName}`, null),
    };
};
