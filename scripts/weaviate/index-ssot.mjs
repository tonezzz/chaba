...
async function createCollection() {
  try {
    const schema = JSON.parse(await readFile(join(__dirname, "schema.json"), "utf-8"));
...
function extractTags(content, type) {
  ...
  const keywords = content.match(
    /\b(apps|infrastructure|gpu|docker|api|database|weaviate|postgres|redis|yomi|track4|imagen|video)\b/gi
  );
  if (keywords) {
    keywords.forEach((kw) => {
...
async function indexDocument(filePath, type, category) {
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      console.log(
        `Embedding generated for chunk ${i + 1}/${chunks.length}: ${embedding.length} dimensions`
      );
