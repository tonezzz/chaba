...
class SearchAnalytics {
    ...
    if (logs.length > 0) {
      ...
      logs.forEach((log) => {
        ...
        if (log.metadata.searchTime) {
          metrics.searchTimeStats.min = Math.min(
            metrics.searchTimeStats.min,
            log.metadata.searchTime
          );
          metrics.searchTimeStats.max = Math.max(
            metrics.searchTimeStats.max,
            log.metadata.searchTime
          );
        }
      });
      ...
      const searchTimes = logs
        .filter((log) => log.metadata.searchTime)
        .map((log) => log.metadata.searchTime);
  ...
  async searchQualityReport() {
    ...
    console.log(
      `  Min: ${metrics.searchTimeStats.min === Infinity ? "N/A" : metrics.searchTimeStats.min + "ms"}`
    );
    console.log(
      `  Max: ${metrics.searchTimeStats.max === 0 ? "N/A" : metrics.searchTimeStats.max + "ms"}`
    );
    ...
    recentSearches.forEach((search) => {
      console.log(
        `  [${search.timestamp}] "${search.query.substring(0, 40)}..." (${search.resultCount} results)`
      );
    });
...
async function testSearchAnalytics() {
  ...
  await analytics.logSearch("GPU queue system", [{ id: "1" }, { id: "2" }], {
    searchTime: 15,
    chunkSize: 512,
  });
