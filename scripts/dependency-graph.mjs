...
function generateMermaidGraph(improvements) {
  ...
  improvements.forEach((imp) => {
    ...
    const statusColor =
      imp.status === "completed" ? "#90EE90" : imp.status === "pending" ? "#FFD700" : "#87CEEB";
    const priorityShape =
      imp.priority === "high" ? "diamond" : imp.priority === "medium" ? "rect" : "circle";
...
function generateDotGraph(improvements) {
  ...
  improvements.forEach((imp) => {
    const nodeId = imp.label.replace(/[^a-zA-Z0-9]/g, "_");
    const statusColor =
      imp.status === "completed" ? "lightgreen" : imp.status === "pending" ? "gold" : "lightblue";
