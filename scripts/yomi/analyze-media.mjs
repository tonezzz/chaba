...
const GEMINI_VISION_MODEL_FALLBACK =
  process.env.GEMINI_VISION_MODEL_FALLBACK || "gemma-4-26b-a4b-it";
...
async function analyzeMedia() {
  ...
  try {
    ...
    if (contextRows.length > 0) {
      ...
      const sortedContext = contextRows.sort(
        (a, b) => (a.delivered_time || a.created_at || 0) - (b.delivered_time || b.created_at || 0)
      );
      ...
      const beforeMessages = sortedContext
        .filter((msg) => (msg.delivered_time || msg.created_at || 0) < targetTimestamp)
        .slice(-3); // Last 3 before target

      const afterMessages = sortedContext
        .filter((msg) => (msg.delivered_time || msg.created_at || 0) > targetTimestamp)
      ...
      console.log(
        `Fetched ${contextRows.length} context messages, using ${allContextMessages.length} (before: ${beforeMessages.length}, after: ${afterMessages.length})`
      );
    }
    ...
    const languageInstructions = {
      ...
      english:
        "Answer briefly (max 20 words) what the media shows, considering conversation context. Put your final clean answer in the last paragraph, and include all your analysis in the paragraphs before it",
      mixed:
        "ตอบสั้นๆ (ไม่เกิน 20 คำ) ว่าสื่อแสดงอะไร พิจารณาบริบทเรื่องด้วย ใส่คำตอบสุดท้ายในย่อหน้าสุดท้าย และใส่การวิเคราะห์ทั้งหมดในย่อหน้าก่อนหน้า",
      unknown:
        "ตอบสั้นๆ (ไม่เกิน 20 คำ) ว่าสื่อแสดงอะไร พิจารณาบริบทเรื่องด้วย ใส่คำตอบสุดท้ายในย่อหน้าสุดท้าย และใส่การวิเคราะห์ทั้งหมดในย่อหน้าก่อนหน้า", // Default to Thai for unknown language
    };
    ...
    for (const modelToTry of [GEMINI_VISION_MODEL_PRIMARY, GEMINI_VISION_MODEL_FALLBACK]) {
      try {
        ...
        const textContext = messageText
          ? `The message also contains this text: "${messageText}"`
          : "The message has no accompanying text.";
...
${languageInstruction}`;
        ...
        if (
          ...
          if (modelToTry === GEMINI_VISION_MODEL_PRIMARY) {
            console.log(
              `Rate limit hit on ${modelToTry}, trying fallback model ${GEMINI_VISION_MODEL_FALLBACK}`
            );
