...
import {
  ...
  dailyCircuitBreaker,
} from "./llama-rate-limiter.mjs";
import {
  submitSummaryJob,
  submitDailySummaryJob,
  submitBatchDailySummaryJob,
  waitForJob,
} from "./gpu-queue-integration.mjs";
import pool from "./db.mjs";
import {
  geminiConversationSummary,
  geminiDailySummary,
  geminiBatchDailySummary,
} from "./gemini-integration.mjs";
...
const MESSAGES_DIR =
  "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/messages";
const MEDIA_DIR = "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/media";
const SUMMARY_CACHE =
  process.env.SUMMARY_CACHE ||
  "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/summaries.json";
...
function parseCSVMessages(text, headers) {
  ...
  for (const rawLine of text.split("\n")) {
    ...
    for (let i = 0; i < headers.length; i++) {
      ...
      if (
        /^(createdTime|deliveredTime)$/.test(key) &&
        typeof value === "string" &&
        /^-?\d+$/.test(value)
      ) {
...
async function downloadMedia(chatId, messageId) {
  ...
  const cached = existsSync(dir)
    ? readdirSync(dir).find((f) => f.startsWith(`${messageId}.`)) || null
    : null;
  if (cached) {
    const ext = cached.split(".").pop().toLowerCase();
    const mime =
      Object.keys(MIME_TO_EXT).find((k) => MIME_TO_EXT[k] === ext) || "application/octet-stream";
    return { fileName: cached, mime };
  }
  const result = await client.callTool({
    name: "get_message_media",
    arguments: { chatId, messageId, preview: false },
  });
...
async function getChatMessages(chatId, pageSize = 100) {
  ...
  const mediaMessages = all.filter((m) =>
    /^(image|video|audio|file)$/.test(String(m.mediaType || "").toLowerCase())
  );
...
function buildPrompt(name, messages) {
  const sorted = [...messages].sort(
    (a, b) =>
      (normalizeTimestamp(a.deliveredTime) || 0) - (normalizeTimestamp(b.deliveredTime) || 0)
  );
  const lines = sorted
    .slice(-40)
    .map((m) => {
...
async function summarizeWithLlama(prompt, chatId = "unknown") {
  ...
  if (!contextStatus.valid) {
    console.error(
      `Context length validation failed: ${contextStatus.length} > ${contextStatus.limit}`
    );
    throw new Error(
      `Context length ${contextStatus.length} exceeds maximum ${contextStatus.limit}`
    );
  }
  ...
  try {
    ...
    console.log(
      `GPU queue failed for ${chatId}: ${queueError.message}, falling back to direct API`
    );
          ...
          const res = await fetch(LLAMA_URL, {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "system",
                  content: "You write concise one-sentence summaries of chat conversations.",
                },
...
async function getSummary(chatId, messages, name, forceRefresh = false) {
  const lastTime = messages.reduce(
    (max, m) => Math.max(max, normalizeTimestamp(m.deliveredTime) || 0),
    0
  );
  ...
  try {
    const summary = await retryWithBackoff(() => summarizeWithLlama(prompt), {
      maxRetries: 3,
      baseDelay: 1000,
      maxDelay: 10000,
      onRetry: (attempt, delay, error) => {
        console.log(
          `Summary retry ${attempt}/3 for ${chatId} after ${delay}ms (error: ${error.message})`
        );
...
function groupMessagesByDate(messages) {
  ...
  for (const m of messages) {
    ...
    console.log(
      `groupMessagesByDate: timestamp=${normalizedTime}, date=${date}, text=${m.text?.substring(0, 30)}`
    );
...
async function extractDailyWithLlama(prompt, chatId = "unknown", date = "unknown") {
  ...
  try {
    ...
    console.log(
      `GPU queue failed for daily summary ${chatId} on ${date}: ${queueError.message}, falling back to direct API`
    );
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You extract structured information from chat conversations and return valid JSON.",
                },
                { role: "user", content: validatedPrompt },
              ],
              temperature: 0.3,
...
async function extractBatchDailyWithLlama(prompt, chatId = "unknown", dates = []) {
  ...
  try {
    console.log(
      `Attempting GPU queue for batch daily summary of ${chatId} (${dates.length} dates)`
    );
    const jobId = await submitBatchDailySummaryJob(
      chatId,
      dates,
      validatedPrompt,
      "yomi_daily_batch"
    ...
    console.log(
      `GPU queue failed for batch daily summary ${chatId}: ${queueError.message}, falling back to direct API`
    );
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You extract structured information from chat conversations and return valid JSON with date keys.",
                },
                { role: "user", content: validatedPrompt },
              ],
              temperature: 0.3,
...
async function saveDailySummary(chatId, date, events, actions, topics, messageCount) {
  ...
  console.log(
    `saveDailySummary: events=${events.length}, actions=${actions.length}, topics=${topics.length}, messageCount=${messageCount}`
  );
...
function prioritizeDates(datesArray, mode, limit) {
  ...
  if (mode === "recent") {
    ...
    return datesArray.sort((a, b) => b[0].localeCompare(a[0])).slice(0, limit);
  }

  if (mode === "high-activity") {
    // Sort by message count descending (most active first)
    return datesArray.sort((a, b) => b[1].length - a[1].length).slice(0, limit);
  }
...
async function generateDailySummaries(chatId, messages, name, targetDate = null) {
  ...
  console.log(
    `Filtered to ${filteredDates.size} dates within last 30 days (skipped ${byDate.size - filteredDates.size} older dates)`
  );
  ...
  if (targetDate) {
    ...
    if (targetMessages) {
      ...
      console.log(
        `Target date ${targetKey} not found in messages. Available dates: ${Array.from(filteredDates.keys()).join(", ")}`
      );
  ...
  if (USE_GEMINI && GEMINI_PRIORITY_MODE !== "all") {
    ...
    console.log(
      `Gemini priority mode: ${GEMINI_PRIORITY_MODE}, processing ${datesArray.length} dates (limit: ${GEMINI_MAX_DATES})`
    );
  } else {
  ...
  for (let i = 0; i < datesArray.length; i += batchSize) {
    ...
    try {
      ...
      if (batch.length > 1) {
        console.log(
          `Processing batch for ${chatId}: ${batchDates.join(", ")} (${batch.length} dates)`
        );
          ...
          try {
            ...
            for (const [date, dayMessages] of batch) {
              ...
              if (dateData) {
                ...
                processed++;
                console.log(
                  `Daily summary generated for ${chatId} on ${date} (batch: ${processed}/${byDate.size} complete)`
                );
              } else {
                console.log(
                  `No data for ${date} in batch response, falling back to single processing`
                );
            ...
            console.log(
              `Batch processing failed: ${batchErr.message}, falling back to single-date processing`
            );
  ...
  console.log(
    `Daily summary generation complete for ${chatId}: ${processed}/${filteredDates.size} days processed`
  );
}
...
async function processSingleDate(chatId, date, dayMessages, name, total, processed) {
  try {
    ...
    console.log(
      `Daily summary generated for ${chatId} on ${date} (${processed + 1}/${total} complete)`
    );
  } catch (err) {
...
async function loadMessages(chatId) {
  const { rows } = await pool.query(
    "SELECT data FROM messages WHERE chat_id = $1 ORDER BY delivered_time ASC",
    [chatId]
  );
...
async function loadConversations() {
  const { rows } = await pool.query(
    'SELECT chat_id AS id, name, is_group AS "isGroup", category, category_source AS "categorySource", unread, last_message_time AS "lastMessageTime", last_preview AS "lastPreview", summary, meta FROM conversations'
  );
  return rows;
}
...
async function loadConversation(chatId) {
  const { rows } = await pool.query(
    'SELECT chat_id AS id, name, is_group AS "isGroup", category, category_source AS "categorySource", unread, last_message_time AS "lastMessageTime", last_preview AS "lastPreview", summary, meta FROM conversations WHERE chat_id = $1',
    [chatId]
  );
  return rows[0] || null;
}
...
async function refreshSingle(chatId, forceRefresh = false, targetDate = null) {
  ...
  if (conv) {
    conv.lastMessageTime = messages.reduce(
      (max, m) => Math.max(max, normalizeTimestamp(m.deliveredTime) || 0),
      0
    );
    const byTimeDesc = [...messages].sort(
      (a, b) =>
        (normalizeTimestamp(b.deliveredTime) || 0) - (normalizeTimestamp(a.deliveredTime) || 0)
    );
    const lastPreviewMsg = byTimeDesc.find(
      (m) => m.text != null || (m.mediaType && m.mediaType !== "unavailable")
    );
    conv.lastPreview = lastPreviewMsg
      ? (lastPreviewMsg.text ?? `[${lastPreviewMsg.mediaType.toLowerCase()}]`)
      : null;
...
async function exportAll() {
  ...
  for (const c of records) {
    try {
      ...
      c.lastMessageTime = messages.reduce(
        (max, m) => Math.max(max, normalizeTimestamp(m.deliveredTime) || 0),
        0
      );
      const byTimeDesc = [...messages].sort(
