...
async function handleSessionStatus(res) {
  try {
    const { code, out, err } = await spawnNode(`${SCRIPT_DIR}/fetch-conversations.mjs`, [
      "--limit",
      "1",
    ]);
    ...
    if (code === 0) {
      ...
      sessionValid =
        !output.includes("session validation failed") &&
    ...
    let loginAttempts = [];
    const LOGIN_ATTEMPT_FILE =
      "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/fetch-data/login-attempts.json";
    try {
...
async function handleActivityStatus(res) {
  try {
    ...
    try {
      ...
      const [util, memUsed, memTotal, temp] = gpuOutput
        .trim()
        .split(",")
        .map((s) => s.trim());
      gpuStatus = {
    ...
    const anyCircuitBreakerOpen =
      rateLimiterStatus.summary.circuitBreaker.state === "open" ||
    ...
    const healthStatus = {
      ...
      message: anyCircuitBreakerOpen
        ? "Circuit breaker active - GPU overloaded"
        : highGpuTemp
...
async function handleMedia(chatId, messageId, res) {
  ...
  const { code, out, err } = await spawnNode(`${SCRIPT_DIR}/download-media.mjs`, [
    chatId,
    messageId,
  ]);
  if (code !== 0) {
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/yomi/login" && req.method === "POST") {
    try {
      const { code, out, err } = await spawnNode(
        "node",
        ["/home/tony/.local/share/npm-global/node_modules/@rikaidev/yomi/bin/yomi.js", "login"],
        {
