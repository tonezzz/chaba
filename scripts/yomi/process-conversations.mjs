...
import {
  ...
  generateCacheKey,
  parseCacheKey,
  validateAndTruncateContext,
  getContextLengthStatus,
...
const FETCH_DIR =
  "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/fetch-data";
const SUMMARY_CACHE =
  "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/summaries.json";
const STATUS_FILE =
  "/home/tony/CascadeProjects/chaba-tony-dell/stacks/web/public/apps/yomi/process-status.json";
...
function isCommercialService(chatId, name) {
  ...
  for (const pattern of commercialPatterns) {
    if (pattern.test(name)) {
      console.log(
        `Excluding commercial service ${name} (${chatId}) from daily summarization (pattern match)`
      );
      return true;
    }
  }
...
async function summarizeWithLlama(prompt, chatId = "unknown") {
  ...
  // Use direct API with rate limiting
...
async function processSingle(chatId, forceSummarize = false) {
  ...
  const { rows } = await pool.query("SELECT * FROM conversations WHERE chat_id = $1", [chatId]);
  ...
  conv.lastPreview = lastPreviewMsg
    ? (lastPreviewMsg.text ?? `[${lastPreviewMsg.mediaType?.toLowerCase() || "media"}]`)
    : null;
...
async function processAll(forceSummarize = false) {
  ...
  const files = readdirSync(FETCH_DIR).filter(
    (f) => f.endsWith(".json") && f !== "fetch-metadata.json"
  );
  ...
  console.log(
    `Processing ${conversations.length} conversations (prioritized: one-on-one + recent first)...`
  );
  ...
  for (let i = 0; i < conversations.length; i += BATCH_SIZE) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(conversations.length / BATCH_SIZE)} (${batch.length} conversations)...`
    );
  ...
  console.log(
    `Processing complete: ${successCount} succeeded, ${failCount} failed, avg quality: ${avgQuality}`
  );
...
async function shouldSkipProcessing() {
  const skipIntervalHours = parseInt(process.env.YOMI_PROCESS_SKIP_HOURS || "12", 10);
  const status = loadProcessStatus();

  if (!status || !status.timestamp || status.status !== "complete") {
    return false;
  }

  const lastProcessTime = new Date(status.timestamp);
  const now = new Date();
  const hoursSinceLastProcess = (now - lastProcessTime) / (1000 * 60 * 60);

  if (hoursSinceLastProcess < skipIntervalHours) {
    console.log(
      `Skipping processing - last successful processing was ${Math.floor(hoursSinceLastProcess)} hours ago (skip interval: ${skipIntervalHours} hours)`
    );
    return true;
  }
...
async function main() {
  ...
  // Skip processing if recent successful processing exists, unless --force is specified
  if (chatIdx === -1 && !forceSummarize && (await shouldSkipProcessing())) {
    return;
  }
