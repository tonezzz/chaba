...
async function fetchSingle(chatId, retryCount = 0, maxRetries = 5) {
  ...
  try {
    ...
    const lastMessageTime = messages.reduce(
    ...
    if (isRateLimitError(err) && retryCount < maxRetries) {
      ...
      console.error(
        `Rate limit detected for ${chatId}. Retrying in ${Math.floor(delay / 60000)} minutes (attempt ${retryCount + 1}/${maxRetries})...`
      );
      await sleep(delay);
...
async function fetchAll(limit = 200, retryCount = 0, maxRetries = 5) {
  ...
  try {
    ...
    for (let i = 0; i < records.length; i += BATCH_SIZE) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(records.length / BATCH_SIZE)}...`
      );
    ...
    if (isRateLimitError(err) && retryCount < maxRetries) {
      const delay = backoffDelays[Math.min(retryCount, backoffDelays.length - 1)];
      console.error(
        `Rate limit detected fetching conversation list. Retrying in ${Math.floor(delay / 60000)} minutes (attempt ${retryCount + 1}/${maxRetries})...`
      );
      await sleep(delay);
...
async function shouldSkipFetch() {
  ...
  if (minutesSinceLastFetch < skipIntervalMinutes) {
    console.log(
      `Skipping fetch - last successful fetch was ${Math.floor(minutesSinceLastFetch)} minutes ago (skip interval: ${skipIntervalMinutes} minutes)`
    );
    return true;
  }
...
function shouldAttemptLogin() {
  ...
  if (recentAttempts.length >= MAX_LOGIN_ATTEMPTS_PER_HOUR) {
    console.log(
      `Skipping login - ${recentAttempts.length} login attempts in the last hour (max: ${MAX_LOGIN_ATTEMPTS_PER_HOUR})`
    );
    return false;
  }
...
async function validateSession() {
  try {
    ...
    if (
      errorStr.includes("no persisted line session") ||
      ...
      await sendNotification(
        "Yomi Session Expired",
        "LINE session expired. Run: npx @rikaidev/yomi login"
      );
      return false;
    }
