...
function testLanguageDetection() {
  ...
  for (const { text, expected } of testCases) {
    ...
    console.log(
      `${status} "${text.substring(0, 30)}${text.length > 30 ? "..." : ""}" -> ${result} (expected: ${expected})`
    );
...
function testConversationLanguageDetection() {
  ...
  const testCases = [
    {
      messages: [{ text: "Hello" }, { text: "How are you?" }, { text: "I am fine" }],
      expected: "english",
    },
    {
      messages: [{ text: "สวัสดี" }, { text: "เป็นอย่างไรบ้าง" }, { text: "ผมสบายดีครับ" }],
      expected: "thai",
    },
    {
      messages: [{ text: "Hello" }, { text: "สวัสดี" }, { text: "How are you?" }],
      expected: "mixed",
    },
    {
      messages: [],
...
async function testGeminiIntegration() {
  ...
  const hasLanguageParam =
    dailySummaryStr.includes("language") && batchSummaryStr.includes("language");
...
async function main() {
  ...
  const allPassed =
    detectionPassed && conversationDetectionPassed && promptsPassed && integrationPassed;
