...
export function getQueryMetrics() {
  const avgExecutionTime =
    queryMetrics.totalQueries > 0 ? queryMetrics.totalExecutionTime / queryMetrics.totalQueries : 0;
...
export async function getPoolStats() {
  ...
  return {
    ...
    utilization:
      totalCount > 0 ? (((totalCount - idleCount) / totalCount) * 100).toFixed(2) + "%" : "0%",
  };
}
...
function optimizePoolConfig(loadFactor = 1.0) {
  ...
  console.log(
    `Optimizing pool config: max ${poolConfig.max} -> ${newMax}, min ${poolConfig.min} -> ${newMin}`
  );
