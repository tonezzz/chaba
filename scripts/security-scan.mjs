...
const nodeDirectories = [".", "scripts/embeddings", "scripts/gpu-queue", "scripts/weaviate"];
...
async function auditPythonDependencies() {
  ...
  for (const reqFile of pythonRequirements) {
    ...
    try {
      ...
      const result = execSync(`pip-audit -r ${fullPath} --format json`, {
        encoding: "utf8",
        timeout: 90000,
...
async function auditNodeDependencies() {
  ...
  for (const dir of nodeDirectories) {
    ...
    try {
      ...
      const result = execSync(`cd ${fullPath} && npm audit --json`, {
        encoding: "utf8",
        timeout: 90000,
        stdio: ["ignore", "pipe", "pipe"],
      });
...
async function checkContainerImageAges() {
  ...
  for (const image of dockerImages) {
    try {
      ...
      const result = execSync(`docker inspect ${image} --format='{{.Created}}'`, {
        encoding: "utf8",
        timeout: 10000,
      });
...
function generateSecuritySummary() {
  const totalDockerVulns = securityResults.dockerImages.reduce(
    (sum, img) => sum + (img.vulnerabilityCount || 0),
    0
  );
  const totalPythonVulns = securityResults.pythonDependencies.reduce(
    (sum, dep) => sum + (dep.vulnerabilityCount || 0),
    0
  );
  const totalNodeVulns = securityResults.nodeDependencies.reduce(
    (sum, dep) => sum + (dep.vulnerabilityCount || 0),
    0
  );
  ...
  return {
    ...
    overallStatus:
      totalDockerVulns + totalPythonVulns + totalNodeVulns === 0 && staleImages === 0
        ? "secure"
        : "needs-attention",
  };
}
