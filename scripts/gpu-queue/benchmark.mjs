...
async function benchmarkEmbeddingService() {
  ...
  for (let i = 0; i < BENCHMARK_CONFIG.embedding.iterations; i++) {
    ...
    console.log(
      `  Iteration ${i + 1}: ${duration}ms (${data.dimensions} dimensions, ${data.vram_usage_mb}MB VRAM)`
    );
  }
  ...
  const singleStats = {
    avg_duration:
      results.single_embeddings.reduce((sum, r) => sum + r.duration_ms, 0) /
      results.single_embeddings.length,
    ...
    avg_vram:
      results.single_embeddings.reduce((sum, r) => sum + r.vram_usage_mb, 0) /
      results.single_embeddings.length,
  };
  console.log(
  ...
  for (let i = 0; i < BENCHMARK_CONFIG.embedding.iterations; i++) {
    ...
    console.log(
      `  Iteration ${i + 1}: ${duration}ms total (${results.batch_embeddings[i].per_text_ms.toFixed(1)}ms per text, ${data.count} texts)`
    );
  }
  ...
  const batchStats = {
    avg_duration:
      results.batch_embeddings.reduce((sum, r) => sum + r.duration_ms, 0) /
      results.batch_embeddings.length,
    ...
    avg_per_text:
      results.batch_embeddings.reduce((sum, r) => sum + r.per_text_ms, 0) /
      results.batch_embeddings.length,
  };
  console.log(
...
async function benchmarkQueueSystem() {
  ...
  try {
    ...
    stats.forEach((stat) => {
      if (stat.count > 0) {
        console.log(
          `  ${stat.type}/${stat.status}: ${stat.count} jobs (avg execution: ${stat.avg_execution_time_ms ? Math.round(stat.avg_execution_time_ms) + "ms" : "N/A"}, avg queue wait: ${stat.avg_queue_wait_time_ms ? Math.round(stat.avg_queue_wait_time_ms) + "ms" : "N/A"})`
        );
      }
    });
    ...
    const completedJobs = stats.reduce(
      (sum, s) => sum + (s.status === "completed" ? s.count : 0),
      0
    );
  ...
  try {
    ...
    cancellationRate.forEach((rate) => {
      console.log(
        `  ${rate.type}: ${rate.cancellation_rate}% (${rate.cancelled}/${rate.total} cancelled)`
      );
    });
  } catch (error) {
  ...
  try {
    ...
    failures.forEach((failure) => {
      console.log(
        `  Job ${failure.id} (${failure.type}): ${failure.status} - ${failure.error || "No error"}`
      );
    });
  } catch (error) {
...
async function benchmarkWeaviateSearch() {
  ...
  for (const query of testQueries) {
    ...
    console.log(
      `  "${query.substring(0, 40)}...": ${duration}ms (${data.objects?.length || 0} results)`
    );
  }
  ...
  const searchStats = {
    avg_duration:
      results.search_queries.reduce((sum, r) => sum + r.duration_ms, 0) /
      results.search_queries.length,
    ...
    avg_results:
      results.search_queries.reduce((sum, r) => sum + r.result_count, 0) /
      results.search_queries.length,
  };
  console.log(
  ...
  const summary = {
    ...
    queue_system: {
      ...
      completion_rate: queueResults.job_stats
        ? (
            (queueResults.job_stats.reduce(
              (sum, s) => sum + (s.status === "completed" ? s.count : 0),
              0
            ) /
              queueResults.job_stats.reduce((sum, s) => sum + s.count, 0)) *
            100
          ).toFixed(2)
        : "N/A",
      avg_queue_wait_ms:
        queueResults.job_stats?.reduce((sum, s) => sum + (s.avg_queue_wait_time_ms || 0), 0) /
          queueResults.job_stats.length || 0,
    },
    weaviate_search: {
...
export {
  ...
  benchmarkEmbeddingService,
  benchmarkQueueSystem,
  benchmarkWeaviateSearch,
};
