...
export async function processJob(job) {
  ...
  while (retryCount <= MAX_RETRIES) {
    try {
      ...
      console.log(
        `Job ${job.id} completed successfully in ${executionTime}ms (attempt ${retryCount + 1})`
      );
      return;
    } catch (error) {
      lastError = error;
      ...
      if (isRetryable && retryCount < MAX_RETRIES) {
        retryCount++;
        console.log(
          `Retrying job ${job.id} in ${RETRY_DELAY}ms... (attempt ${retryCount + 1}/${MAX_RETRIES})`
        );
...
async function processEmbeddingJob(job) {
    ...
    if (mcpGpu) {
      try {
        ...
        if (vramFree < vramRequired) {
          console.warn(
            `Insufficient VRAM: ${vramFree}MB free, ${vramRequired}MB required. Falling back to CPU.`
          );
  ...
  try {
    ...
    throw new Error(
      `Embedding generation failed: ${error.message} (service: ${embeddingServiceUrl}, gpu: ${params.use_gpu})`
    );
  }

  const endTime = Date.now();
  console.log(
    `Embeddings generated in ${endTime - startTime}ms (${params.use_gpu ? "GPU" : "CPU"})`
  );
  ...
  while (true) {
    try {
      ...
      if (!busy) {
        ...
        if (job) {
          console.log(
            `Found pending job ${job.id} (${scheduler.getScheduler()} scheduler), processing...`
          );
