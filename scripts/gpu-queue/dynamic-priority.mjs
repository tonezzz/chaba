  ...
  for (const job of pendingJobs) {
    ...
    if (waitTime > maxWaitMs && job.priority < 5) {
      try {
        ...
        escalated++;
        console.log(
          `Escalated job ${job.id} to critical priority (waited ${Math.floor(waitTime / 60000)} minutes)`
        );
      } catch (error) {
...
export async function getPriorityAdjustmentStats() {
  ...
  const averageDynamicPriority =
    dynamicPriorities.reduce((sum, dp) => sum + dp.dynamicPriority, 0) / dynamicPriorities.length;
