...
server.listen(PORT, HOST, () => {
  ...
  console.log(
    "  GET    /api/gpu-queue/intelligent-metrics         - Comprehensive intelligent metrics"
  );
  console.log(
    "  GET    /api/gpu-queue/scheduling-performance       - Scheduling performance comparison"
  );
...
async function startQueueProcessor() {
  ...
  while (true) {
    try {
      ...
      console.log(
        `GPU Status: ${gpuStatus.memoryPercent.toFixed(1)}% used, Available: ${gpuStatus.available}`
      );
      ...
      if (job) {
        ...
        try {
          ...
          consecutiveFailures++;
          console.error(
            `Job ${job.id} processing failed (attempt ${consecutiveFailures}/${MAX_CONSECUTIVE_FAILURES}):`,
            error
          );
          ...
          if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
            console.log(
              `Circuit breaker triggered after ${consecutiveFailures} failures, waiting ${BACKPRESSURE_DELAY / 1000}s`
            );
            await new Promise((resolve) => setTimeout(resolve, BACKPRESSURE_DELAY));
            consecutiveFailures = 0;
          }
