...
export async function getSchedulingPerformanceComparison() {
  try {
    ...
    const efficiency =
      avgQueueWait > 0 ? (avgExecutionTime / (avgExecutionTime + avgQueueWait)) * 100 : 0;
...
export async function getMemoryUtilizationTrends(hours = 24) {
  try {
    ...
    const peakUtilization =
      trends.length > 0 ? Math.max(...trends.map((t) => (t.peak / gpuConfig.total) * 100)) : 0;
...
function generateRecommendations(metrics, performance) {
  ...
  if (metrics.memory.utilization > 80) {
    recommendations.push({
      ...
      severity: "warning",
      message:
        "GPU memory utilization is high (>80%). Consider adjusting job priorities or reducing batch sizes.",
    });
  }
  ...
  if (parseFloat(metrics.prediction.accuracy) < 70) {
    recommendations.push({
      ...
      severity: "warning",
      message:
        "Prediction accuracy is below 70%. Consider retraining the prediction model or switching to a different algorithm.",
    });
  }
  ...
  if (performance.avgQueueWait > 60000) {
    recommendations.push({
      ...
      severity: "warning",
      message:
        "Average queue wait time exceeds 60 seconds. Consider increasing GPU resources or optimizing job scheduling.",
    });
  }
  ...
  if (parseFloat(performance.throughput) < 0.5) {
    recommendations.push({
      ...
      severity: "info",
      message:
        "Job throughput is low. Consider batching similar jobs or adjusting scheduling algorithm.",
    });
  }
