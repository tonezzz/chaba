...
if (suggestions.priorityConflicts.length > 0) {
  ...
  suggestions.priorityConflicts.forEach((conflict) => {
    console.log(
      `⚠️ ${conflict.improvement} (${conflict.currentPriority}) depends on ${conflict.dependency} (${conflict.depPriority})`
    );
...
if (suggestions.orphanImprovements.length > 0) {
  ...
  suggestions.orphanImprovements.forEach((imp) => {
    console.log(`🔄 ${imp.label} (${imp.priority})`);
    console.log(
      `   Suggested action: Consider if this should depend on or block other improvements\n`
    );
  });
}
...
function generateSuggestions(improvements) {
  ...
  for (const improvement of improvements) {
    ...
    if (
      improvement.label.includes("Guide") ||
      improvement.label.includes("Rules") ||
      improvement.label.includes("Best Practices") ||
      improvement.label.includes("Example")
    ) {
      continue;
    }
    ...
    const hasDependents = improvements.some(
      (imp) => imp.depends_on && imp.depends_on.includes(improvement.label)
    );
  ...
  for (const improvement of improvements) {
    ...
    for (const rule of categoryRules) {
      if (improvement.category === rule.category) {
        ...
        const potentialDeps = improvements.filter(
          (imp) =>
