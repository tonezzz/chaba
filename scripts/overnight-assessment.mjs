...
const {
  Client,
} = require("/home/tony/CascadeProjects/chaba/mcp-servers/mcp-health/node_modules/@modelcontextprotocol/sdk/dist/cjs/client/index.js");
const {
  StdioClientTransport,
} = require("/home/tony/CascadeProjects/chaba/mcp-servers/mcp-health/node_modules/@modelcontextprotocol/sdk/dist/cjs/client/stdio.js");
...
function loadPerformanceBaselines() {
  const baselinePath =
    "/home/tony/CascadeProjects/chaba/docs/ssot/infrastructure/performance-baselines.yml";
  ...
  if (analysis.within_baseline.length > 0) {
    ...
    if (examples.length > 0) {
      ...
      examples.forEach((example) => {
        const deviation =
          example.deviation > 0 ? `+${example.deviation}%` : `${example.deviation}%`;
    ...
    if (result.content && result.content.length > 0) {
      ...
      if (!Array.isArray(historyData)) {
        ...
        if (historyData.history && Array.isArray(historyData.history)) {
          ...
          console.log(
            "MCP health history data is not in expected format, skipping historical analysis"
          );
          return null;
        }
      }
...
function generateHistoricalTrendReport(healthHistory, alerts) {
  ...
  const degradedServices = services.filter(
    (s) => s.healthy_percentage >= 80 && s.healthy_percentage < 95
  ).length;
  ...
  if (alerts) {
    ...
    if (criticalAlerts.length > 0) {
      ...
      criticalAlerts.forEach((service) => {
        ...
        content += `- **Alert Types:** ${Object.entries(service.alert_types)
          .map(([type, info]) => `${type} (${info.severity})`)
          .join(", ")}\n`;
...
function autoCreateKBEntry(title, content, tags = []) {
  ...
  const filename = `${title
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/[^a-z0-9-]/g, "")}-${timestamp.split("T")[0]}.md`;
    ...
    try {
      if (service.type === "http") {
        ...
        if (result.status !== expectedStatus && !service.note) {
          criticalIssues.push(
            `${service.name} returned status ${result.status} (expected ${expectedStatus})`
          );
        ...
        const containerCheck = execCommand(
        ...
        if (containerCheck.success && containerCheck.output.includes("Up")) {
          ...
          const errorMsg = containerCheck.success
...
async function checkGPUStatus() {
  ...
  try {
    ...
    if (thermalOutput.success) {
      const temps = thermalOutput.output
        .trim()
        .split("\n")
...
function checkConfiguration() {
  ...
  const ipCheck = execCommand(
    'grep -r "192\\.168\\." /home/tony/CascadeProjects/chaba/docs/overview/ 2>/dev/null || true'
  );
...
async function checkSecurityStatus() {
  ...
  try {
    const securityCheck = execCommand(
      "node /home/tony/CascadeProjects/chaba/scripts/security-scan.mjs",
      300000
    ); // 5 minute timeout
      ...
      try {
        ...
        if (existsSync(securityResultsPath)) {
          ...
          if (summary.staleContainerImages > 0) {
            mediumIssues.push(
              `${summary.staleContainerImages} stale container images found (>90 days old)`
            );
    ...
    try {
      ...
      if (dependencyValidation.issues.length > 0) {
        ...
        if (dependencyValidation.criticalIssues > 0) {
          highIssues.push(
            "Critical dependency issues found: " + dependencyValidation.criticalIssues
          );
        } else {
          mediumIssues.push(
            "Dependency validation issues found: " + dependencyValidation.issues.length
          );
        }
      } else {
      ...
      if (blockedImprovements.length > 0) {
        ...
        blockedImprovements.forEach((blocked) => {
          content +=
            "- **" + blocked.label + "** blocked by: " + blocked.blockedBy.join(", ") + "\n";
        });
        content += "\n";
      }
      ...
      if (blockingImprovements.length > 0) {
        ...
        blockingImprovements.forEach((blocking) => {
          content +=
            "- **" + blocking.label + "** blocking: " + blocking.blocking.join(", ") + "\n";
        });
        content += "\n";
      }
...
function calculateOverallImpact(improvement) {
  ...
  const user_experience =
    improvement.user_experience_impact !== null ? improvement.user_experience_impact : 5;
  const cost_savings =
    improvement.cost_savings_impact !== null ? improvement.cost_savings_impact : 5;
    ...
    if (improvement.depends_on && improvement.depends_on.length > 0) {
      for (const dep of improvement.depends_on) {
        ...
        if (!improvementLabels.has(dep)) {
          issues.push(
            `Missing dependency: "${improvement.label}" depends on non-existent "${dep}"`
          );
          criticalIssues++;
        }
      ...
      for (const dep of improvement.depends_on) {
        ...
        if (depImprovement && depImprovement.priority) {
          ...
          if (currentPriority > depPriority) {
            issues.push(
              `Priority inconsistency: "${improvement.label}" (${improvement.priority}) depends on lower priority "${dep}" (${depImprovement.priority})`
            );
          }
        }
      }
...
function detectKBWorthyIssues() {
  ...
  if (
    reportLower.includes("hostname") &&
    reportLower.includes("etc/hosts") &&
...
Related issues: ${
      criticalIssues
        .filter((i) => i.includes("hostname") || i.includes("tony-omen"))
        .slice(0, 2)
        .join(", ") || "Network connectivity issues detected"
    }. 
This suggests the /etc/hosts file may have incorrect IP mappings for local hostnames.`;
  ...
  if (
    reportLower.includes("gpu") &&
    reportLower.includes("vram") &&
...
Related issues: ${
      criticalIssues
        .filter((i) => i.includes("gpu") || i.includes("vram"))
        .slice(0, 2)
        .join(", ") || "GPU memory pressure detected"
    }. 
This indicates GPU memory constraints affecting system performance.`;

    if (!kbEntryExists("GPU VRAM Exhaustion")) {
      autoCreateKBEntry("GPU VRAM Exhaustion", content, ["gpu", "memory", "performance", "vram"]);
    }
  }

  // Disk space critical - specific pattern
  if (
    reportLower.includes("disk") &&
...
Related issues: ${
      criticalIssues
        .filter((i) => i.includes("disk") || i.includes("space"))
        .slice(0, 2)
        .join(", ") || "Disk space pressure detected"
    }. 
...
function archiveOldReports() {
  ...
  try {
    ...
    if (archived > 0)
      console.log(`Archived ${archived} report(s) older than ${REPORT_RETENTION_DAYS} days.`);
  } catch (err) {
...
function saveReport(assessmentStartTime) {
  ...
  console.log(
    `Total issues: ${criticalIssues.length} critical, ${highIssues.length} high, ${mediumIssues.length} medium, ${lowIssues.length} low`
  );
  ...
  try {
    ...
    try {
      ...
      if (yomiResult.status === 200) {
        ...
        if (yomiProcessing) {
          console.log(
            "Yomi is actively processing, waiting 30 seconds for GPU load to normalize..."
          );
