    ...
    if (inSections) {
      ...
      if (currentItem && indent === 8) {
        ...
        if (trimmed.startsWith("text:")) {
          ...
          trimmed.startsWith("strategic_value:") ||
          trimmed.startsWith("estimated_duration:") ||
...
function extractFocusNames(items, statusFilter = "active") {
  if (!items) return [];
  return items.filter((item) => item.status === statusFilter).map((item) => item.label);
}
  ...
  if (sharedFocusSection && branchFocusSection) {
    ...
    if (rules.require_priority) {
      ...
      if (missingPriority.length > 0) {
        errors.push(
          `Missing priority field for: ${missingPriority.map((i) => i.label).join(", ")}`
        );
      }
    }
  ...
  for (const section of data.sections) {
    ...
    for (const item of section.items || []) {
      ...
      if (!subagent.host) {
        if (safe) {
          errors.push(
            `[${context}] "${item.label}" is safe-to-parallel/Ready (Safe) but has no subagent.host`
          );
        } else if (requireSubagent) {
          warnings.push(
            `[${context}] "${item.label}" has no subagent block; tony-dell dispatch cannot be verified`
          );
        }
        continue;
      }
      ...
      if (safe) {
        if (subagent.runnable !== true) {
          errors.push(
            `[${context}] "${item.label}" safe-to-parallel item must have subagent.runnable: true`
          );
        }
        if (subagent.requires_approval !== false) {
          errors.push(
            `[${context}] "${item.label}" safe-to-parallel item must have subagent.requires_approval: false`
          );
        }
        if (subagent.host !== "tony_dell") {
          errors.push(
            `[${context}] "${item.label}" safe-to-parallel item must dispatch to tony_dell (got ${subagent.host})`
          );
        }
      }
      ...
      if (
        ...
        subagent.requires_approval === false &&
        !allowed.includes(subagent.host)
      ) {
        warnings.push(
          `[${context}] "${item.label}" runnable subagent uses host "${subagent.host}" not in allowed list`
        );
      }
    }
  }
  ...
  for (const section of data.sections) {
    for (const item of section.items || []) {
      ...
      if (safe && !triage.complication && !(item.subtasks || []).length) {
        warnings.push(
          `[${context}] "${item.label}" is safe-to-parallel but has no triage or subtasks`
        );
      }
      ...
      if (triage.complication !== undefined) {
        ...
        if (Number.isNaN(c) || c < 1 || c > 10) {
          errors.push(
            `[${context}] "${item.label}" triage.complication must be an integer 1-10 (got ${triage.complication})`
          );
        } else if (c > 4 && safe) {
          errors.push(
            `[${context}] "${item.label}" has triage.complication ${c} but is marked safe_to_parallel; must be <= 4`
          );
        }
      }
    }
...
function main() {
  ...
  if (errors.length === 0 && warnings.length === 0) {
    ...
    if (sharedSection) {
      ...
      console.log(
        `   Shared: ${activeShared} active focus(es) - ${activeSharedNames.join(", ") || "None"}`
      );
    }
    ...
    if (branchSection) {
      ...
      console.log(
        `   Branch: ${activeBranch} active focus(es) - ${activeBranchNames.join(", ") || "None"}`
      );
    }
