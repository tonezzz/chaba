...
const {
  Client,
} = require("/home/tony/CascadeProjects/chaba-tony-dell/mcp-servers/mcp-health/node_modules/@modelcontextprotocol/sdk/dist/cjs/client/index.js");
const {
  StdioClientTransport,
} = require("/home/tony/CascadeProjects/chaba-tony-dell/mcp-servers/mcp-health/node_modules/@modelcontextprotocol/sdk/dist/cjs/client/stdio.js");
  ...
  try {
    ...
    const transport = new StdioClientTransport({
      ...
      env: {
        ...
        HEALTH_SKILL:
          "/home/tony/CascadeProjects/chaba-tony-dell/.agents/skills/health-check/SKILL.md",
      },
    });
...
function calculateBaselines(historyData) {
  ...
  Object.keys(byService).forEach((serviceName) => {
    ...
    const responseTimes = healthyChecks
    ...
    const variance =
      responseTimes.reduce((sum, rt) => sum + Math.pow(rt - mean, 2), 0) / responseTimes.length;
    ...
    baselines[serviceName] = {
      ...
      data_quality: {
        ...
        confidence:
          healthyChecks.length >= 5 ? "high" : healthyChecks.length >= 3 ? "medium" : "low",
        date_range: {
...
async function main() {
  ...
  try {
    ...
    const baselineFile =
      "/home/tony/CascadeProjects/chaba-tony-dell/docs/ssot/infrastructure/performance-baselines.yml";
    ...
    Object.values(baselines).forEach((baseline) => {
      console.log(
        `- ${baseline.service_name}: ${baseline.response_time.median}ms median, ${baseline.healthy_percentage.toFixed(1)}% healthy`
      );
    });
  } catch (error) {
