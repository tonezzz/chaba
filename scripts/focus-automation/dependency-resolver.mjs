...
function findCriticalPath(graph) {
  ...
  const maxNode = Object.keys(distances).reduce((a, b) => (distances[a] > distances[b] ? a : b));
...
function findNextActivatable() {
  ...
  if (activatable.length === 0) {
    ...
    const noDeps = allFocuses.filter(
      (f) =>
        f.status !== "active" &&
        f.status !== "completed" &&
  ...
  Object.keys(graph.nodes).forEach((node) => {
    ...
    if (deps.length > 0) {
      ...
      deps.forEach((dep) => {
        ...
        const icon = depStatus === "completed" ? "✅" : depStatus === "active" ? "🔄" : "⏳";
        console.log(`  └─ ${icon} ${dep} (${depStatus})`);
      });
    }
  });
