...
function calculateFocusAffinity(activities, focusName) {
  ...
  activities.forEach((activity) => {
    const activityText =
      `${activity.project} ${activity.branch} ${activity.focus_area || ""} ${JSON.stringify(activity.details)}`.toLowerCase();
...
function generateSuggestions() {
  ...
  console.log(
    `📊 Analyzing ${recentActivities.length} recent activities (${CONFIG.activity_window_hours}h)\n`
  );
  ...
  allFocuses.forEach((focus) => {
    ...
    if (confidence >= CONFIG.min_confidence) {
      ...
      if (dependencyStatus.ready) {
        ...
        reasons.push(
          `Dependencies pending: ${dependencyStatus.blocking_focuses.map((b) => b.name).join(", ")}`
        );
      }
...
function showAffinity() {
  ...
  allFocuses.forEach((focus) => {
    ...
    console.log(
      `   Evidence: ${affinity.evidence.matching_activities}/${affinity.evidence.total_activities} matching activities`
    );
