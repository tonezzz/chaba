...
function showIntegrationStatus() {
  ...
  console.log(
    `   Validation Script: ${existsSync(VALIDATION_SCRIPT) ? "✅" : "❌"} ${VALIDATION_SCRIPT}`
  );
...
function syncWithSSOT() {
  ...
  allFocuses.forEach((focus) => {
    const focusActivities = activityDB.activities.filter(
      (a) => a.focus_area === focus.label || a.details?.focus_context === focus.label
    );
