...
function calculateVelocity(db) {
  ...
  if (recentActivities.length < CONFIG.min_activities_for_velocity) {
    console.log(
      `⚠️  Insufficient activity data for velocity calculation (${recentActivities.length} < ${CONFIG.min_activities_for_velocity})`
    );
    return null;
  }
...
function estimateTimeline(focusData, velocity) {
  ...
  allFocuses.forEach((focus) => {
    ...
    const velocityFactor =
      velocity.activities_per_day > 0 ? Math.log(velocity.activities_per_day + 1) : 1;
...
function identifyBottlenecks(focusData, velocity) {
  ...
  allFocuses.forEach((focus) => {
    const blockingCount = allFocuses.filter(
      (f) => f.dependencies && f.dependencies.includes(focus.label)
    ).length;
    ...
    if (blockingCount > 1) {
      bottlenecks.push({
        ...
        blocked_focuses: allFocuses
          .filter((f) => f.dependencies && f.dependencies.includes(focus.label))
          .map((f) => f.label),
...
function showTimeline() {
  ...
  timeline.forEach((item, index) => {
    const statusIcon = item.status === "active" ? "🔄" : item.status === "completed" ? "✅" : "⏳";
