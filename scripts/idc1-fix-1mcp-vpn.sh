...
echo "[IDC1] Applying 1mcp.idc1.vpn DNS + Caddy fix on $SSH_HOST"

ssh "${SSH_COMMON_OPTS[@]}" "$SSH_USER@$SSH_HOST" \
  "IDC1_STACK_DIR=$IDC1_STACK_DIR COREFILE_REL=$COREFILE_REL CADDYFILE_REL=$CADDYFILE_REL ONE_MCP_PORT=$ONE_MCP_PORT bash -s" <<'EOF'
set -euo pipefail
...
for j in range(hosts_idx, len(block)):
  _depth += block[j].count('{')
  _depth -= block[j].count('}')
  if j > hosts_idx and _depth == 0:
    hosts_end = j
    break
...
CADDYFILE_PATH="$CADDYFILE_PATH" CADDYFILE_TMP="$CADDYFILE_TMP" ONE_MCP_PORT="$ONE_MCP_PORT" python3 - <<'PY'
import os
from pathlib import Path
...
site = '1mcp.idc1.vpn'
if site in text:
  print('[REMOTE] Caddyfile already contains 1mcp.idc1.vpn block.')
  caddyfile_tmp.write_text(text, encoding='utf-8')
  raise SystemExit(0)

port = os.environ.get('ONE_MCP_PORT', '3050')
...
echo "[REMOTE] Completed idc1 1mcp.idc1.vpn fix."
EOF
