...
Sources:
- data/ssot/infrastructure/ssot.ha-devices.yml (generated by build-ha-devices-ui.py)

Targets:
- tony-ha: live push via websocket
...
SSOT_PATH = REPO_ROOT / "data" / "ssot" / "infrastructure" / "ssot.ha-devices.yml"
...
def _table(rows: list[dict]) -> str:
    ...
    for r in rows:
        ...
        if first_seen == "—" and last_seen == "—":
            ...
            seen = f"<small>{first_seen}</small>"
        else:
            seen = f"<small>first: {first_seen}</small><br><small>last: {last_seen}</small>"
        trs.append(
