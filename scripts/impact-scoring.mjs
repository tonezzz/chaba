...
function analyzeMode(improvements) {
  ...
  improvements.forEach((imp) => {
    ...
    console.log(
      `   Business: ${business}, Technical: ${technical}, ` +
...
function scoreMode(improvements) {
  ...
  scoredImprovements.forEach((imp) => {
    ...
    const priorityEmoji =
      imp.suggestedPriority === "HIGH" ? "🔴" : imp.suggestedPriority === "MEDIUM" ? "🟡" : "🟢";

    console.log(
      `${emoji} ${priorityEmoji} ${imp.suggestedPriority} ${imp.overall}/10 - ${imp.label}`
    );
    console.log(
      `   Business: ${imp.business}, Technical: ${imp.technical}, ` +
        `UX: ${imp.user_experience}, Cost: ${imp.cost_savings}`
    );
    console.log(
      `   Manual priority: ${imp.manualPriority}, Status: ${imp.status}, Effort: ${imp.effort}`
    );
    console.log("");
  });
  ...
  if (mismatches.length > 0) {
    ...
    mismatches.forEach((imp) => {
      ...
      console.log(
        `    Manual: ${imp.manualPriority} → Impact-based: ${imp.suggestedPriority} (${imp.overall}/10)`
      );
      console.log("");
    });
  } else {
...
function prioritizeMode(improvements) {
  ...
  top10.forEach((imp, index) => {
    ...
    console.log(
      `${index + 1}. ${emoji} ${priorityEmoji} ${imp.suggestedPriority} ${imp.overall}/10 - ${imp.label}`
    );
    console.log(
      `   Business: ${imp.business}, Technical: ${imp.technical}, ` +
        `UX: ${imp.user_experience}, Cost: ${imp.cost_savings}`
    ...
    console.log(
      `   Manual priority: ${imp.manualPriority}, Status: ${imp.status}, Effort: ${imp.effort}, Category: ${imp.category}`
    );
    console.log("");
  });
  ...
  const highImpactLowEffort = scoredImprovements.filter(
    (imp) =>
      imp.overall >= PRIORITY_THRESHOLDS.HIGH &&
      (imp.effort === "TBD" ||
        imp.effort.toLowerCase().includes("minute") ||
        (imp.effort.toLowerCase().includes("hour") &&
          !imp.effort.includes("2") &&
          !imp.effort.includes("3")))
  );
  ...
  const lowImpactHighEffort = scoredImprovements.filter(
    (imp) =>
  ...
  console.log(
    `  High impact (≥8): ${scoredImprovements.filter((imp) => imp.overall >= PRIORITY_THRESHOLDS.HIGH).length}`
  );
  console.log(
    `  Medium impact (5-7): ${scoredImprovements.filter((imp) => imp.overall >= PRIORITY_THRESHOLDS.MEDIUM_LOW && imp.overall < PRIORITY_THRESHOLDS.HIGH).length}`
  );
  console.log(
    `  Low impact (<5): ${scoredImprovements.filter((imp) => imp.overall < PRIORITY_THRESHOLDS.MEDIUM_LOW).length}`
  );
