...
async function parseBackupLogs() {
  try {
    ...
    if (fs.existsSync(backupDir)) {
      ...
      backupStatus.backupCount = files.filter(
...
async function updateDashboardState() {
  ...
  dashboardState.services = Object.fromEntries(serviceChecks.map((check) => [check.name, check]));

  // Check key containers
  const containers = ["postgres", "redis", "caddy", "gpu-queue"];
  const containerChecks = await Promise.all(containers.map((name) => checkContainerStatus(name)));
...
function generateDashboardHTML() {
  const servicesHTML = Object.entries(dashboardState.services)
    .map(([name, status]) => {
      const statusClass =
      ...
      const responseTime = status.responseTime
        ? `<span class="response-time">${status.responseTime}ms</span>`
        : "";
  ...
  const alertsHTML = dashboardState.alerts
    .slice(-10)
    .map((alert) => {
      const severityClass =
