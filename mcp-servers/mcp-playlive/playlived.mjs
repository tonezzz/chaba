...
async function createSession(type, target, remote_url, reuse_context = false, attach_url = null) {
  const cdpUrl =
    target === "remote" ? remote_url || DEFAULT_REMOTE_CDP : remote_url || DEFAULT_LOCAL_CDP;
  ...
  switch (action) {
    ...
    case "upload": {
      ...
      if (!selector || typeof selector !== "string")
        throw new Error("upload requires a string selector");
      let buf;
      ...
      await page
        .locator(selector)
        .setInputFiles([{ name: uploadFilename, mimeType: uploadMimeType, buffer: buf }]);
      return {
        uploaded: selector,
    ...
    case "drop": {
      const { selector, base64: b64, filename, mimeType, stash_id } = body || {};
      if (!selector || typeof selector !== "string")
        throw new Error("drop requires a string selector");
      ...
      const dropMimeType =
        mimeType || (stash_id ? stash.get(stash_id).mimeType : "application/octet-stream");
    ...
    if (req.method === "POST" && url.pathname === "/sessions") {
      ...
      if (!["chrome-live", "playwright-chrome", "playwright-headless"].includes(type)) {
        return fail(
          res,
          400,
          "type must be chrome-live, playwright-chrome, or playwright-headless"
        );
      }
...
server.listen(PORT, () => {
  ...
  setInterval(
    () => closeIdleSessions().catch((err) => console.error("idle cleanup error", err)),
    60000
  );
});
