...
async function loadHealthConfig() {
  const configPath =
    process.env.HEALTH_CONFIG || join(process.cwd(), "docs/ssot/infrastructure/ssot.health.yml");
  ...
  try {
    ...
    if (process.env.HEALTH_PROFILE) {
      ...
      if (!ssid) {
        try {
          const link = execSync('iw dev wlo1 link 2>/dev/null | grep -E "^\\s+SSID:"', {
            encoding: "utf8",
          });
      ...
      if (profile === "home") {
        try {
          ...
          profile = "mobile";
          try {
            const ip = execSync("ip route get 1.1.1.1 | awk '{print $7}'", {
              encoding: "utf8",
            }).trim();
      ...
      for (const category of profileConfig.categories) {
        if (category.file) {
          ...
          try {
            ...
            console.error(
              `Loaded ${categoryConfig.services?.length || 0} services from category '${category.category}' (${categoryFile})`
            );
          } catch (err) {
...
async function processAlerts(config, serviceName, status, previousStatus, error) {
  ...
  if (
    (status === "healthy" || status === "intentional") &&
    previousStatus === "error" &&
    thresholds.recovery_notification
  ) {
    const existingAlert = await checkExistingAlert(serviceName, "service_failure");
    if (existingAlert) {
...
function getEnhancedErrorContext(serviceName, status, error, serviceType, serviceUrl, config) {
  ...
  if (ssotRecoveryActions.length > 0) {
    troubleshootingSteps.push("--- SSOT Recovery Actions ---", ...ssotRecoveryActions);
  }
...
async function checkPortAvailability(port, serviceProcessName = null) {
  try {
    ...
    if (serviceProcessName) {
      ...
      const isExpectedProcess = processes.some(
        (p) =>
        ...
        if (currentBlock.includes("handle ") && !currentBlock.includes("handle_path ")) {
          ...
          if (blockContent.includes("reverse_proxy") && !blockContent.includes("file_server")) {
            issues.push({
              ...
              block: currentBlock,
              message:
                'Using "handle" instead of "handle_path" may cause path stripping issues with reverse_proxy',
              recommendation:
                'Consider using "handle_path" for reverse_proxy blocks to preserve path information',
            });
          }
        }
        currentBlock = "";
    ...
    for (let i = 0; i < routePatterns.length; i++) {
      for (let j = i + 1; j < routePatterns.length; j++) {
        ...
        if (route1.pattern !== route2.pattern) {
          if (
            route1.pattern.startsWith(route2.pattern) ||
            route2.pattern.startsWith(route1.pattern)
          ) {
            issues.push({
...
async function checkHTTPService(service) {
  ...
  try {
    const response = execSync(
      `curl -s -o /dev/null -w "%{http_code}" --max-time ${service.timeout || 5} "${service.url}"`,
      {
        encoding: "utf8",
        stdio: "pipe",
      }
  ...
  try {
    ...
    try {
      if (remoteHost && !["tony_dell", "tony-dell", "localhost"].includes(remoteHost)) {
        output = execSync(
          `ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=5 ${remoteHost} podman ps -a --filter "name=${service.container}" --format "table {{.Names}}\\t{{.Status}}"`,
          {
            encoding: "utf8",
            stdio: "pipe",
          }
        ...
        output = execSync(
          `podman ps -a --filter "name=${service.container}" --format "table {{.Names}}\\t{{.Status}}"`,
          {
            encoding: "utf8",
            stdio: "pipe",
          }
    ...
    if (!output.trim() || !output.includes("Up")) {
      try {
        if (remoteHost && !["tony_dell", "tony-dell", "localhost"].includes(remoteHost)) {
          output = execSync(
            `ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=5 ${remoteHost} podman ps -a | grep ${service.container}`,
            {
              encoding: "utf8",
              stdio: "pipe",
            }
    ...
    if (healthStatus === "error" && (state === "exited" || state === "dead")) {
      try {
        const containerInfo = execSync(
          `docker inspect ${service.container} --format '{{json .HostConfig.PortBindings}}'`,
          { encoding: "utf8" }
        );
...
async function checkSystemService(service) {
  ...
  const sshPrefix =
    service.host && !localHosts.includes(service.host)
      ? `ssh -o StrictHostKeyChecking=no -o BatchMode=yes ${service.host.replace(/_/g, "-")} `
      : "";

  try {
    const output = execSync(
      `${sshPrefix}${systemctlCmd} show ${service.service} --property=ActiveState --property=SubState`,
      {
        encoding: "utf8",
        stdio: "pipe",
      }
    ...
    let isTimer = false;
    try {
      const serviceType = execSync(
        `${sshPrefix}${systemctlCmd} show ${service.service} --property=Type --value`,
        { encoding: "utf8" }
      );
      isTimer = serviceType.trim() === "oneshot" || serviceType.trim() === "simple";
      const unitFile = execSync(
        `${sshPrefix}${systemctlCmd} show ${service.service} --property=Id --value`,
        { encoding: "utf8" }
      );
    ...
    if (isTimer && subState === "waiting" && activeState === "active") {
      ...
      if (
        ...
        (subState === "running" || subState === "exited" || subState === "waiting")
      ) {
        status = "healthy";
  ...
  return {
    tools: [
        ...
        inputSchema: {
          ...
          properties: {
            port: {
              type: "number",
              description:
                "Optional specific port to check (checks all service ports if not provided)",
            },
          },
        },
      },
      ...
      },
      {
        ...
        description:
          "Safely restart a service with conflict resolution (requires user confirmation)",
        inputSchema: {
          type: "object",
          properties: {
            service_name: {
        ...
        inputSchema: {
          ...
          properties: {
            service: {
              type: "string",
              description:
                "Optional specific service name (checks all critical services if not provided)",
            },
          },
        },
      },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            failure_type: {
              type: "string",
              description:
                "Failure type (container_down, endpoint_unreachable, port_conflict, etc.)",
            },
            enabled: {
              type: "boolean",
    ...
    switch (name) {
      ...
      case "get_health_summary": {
        ...
        const summaryWithUptime = summaryWithCategory.map((item) => ({
          ...item,
          uptime_percentage:
            item.total_checks > 0 ? ((item.healthy_count / item.total_checks) * 100).toFixed(2) : 0,
        }));
      ...
      case "analyze_dependencies": {
        ...
        Object.entries(dependencies).forEach(([service, deps]) => {
          ...
          const deploymentConflicts = depStatuses.filter(
            (d) =>
          ...
          dependencyAnalysis[service] = {
            ...
            potential_cascading_failure:
              serviceStatus?.status !== "healthy" && failingDeps.length > 0,
            dependency_failure_cause:
              failingDeps.length > 0 ? failingDeps.map((d) => d.service).join(", ") : null,
            deployment_conflict_warning:
              deploymentConflicts.length > 0
                ? `Mixed deployment methods detected: ${deploymentConflicts.map((d) => `${d.service}(${d.deployment_method})`).join(", ")}`
                : null,
          };
        });
              ...
              text: JSON.stringify(
                {
                  ...
                  deployment_conflicts_detected: Object.values(dependencyAnalysis).filter(
                    (d) => d.deployment_conflicts.length > 0
                  ).length,
                },
                null,
                2
      ...
      case "restart_service": {
        ...
        const service = config.services?.find(
          (s) => s.name === service_name || s.id === service_name
        );
                ...
                text: JSON.stringify(
                  {
                    ...
                    recommendation:
                      "Kill conflicting processes or use force=true to restart anyway (use with caution)",
                    kill_command: portConflicts
                      .map((c) => `kill ${c.processes?.map((p) => p.pid).join(" ")}`)
                      .join("; "),
                  },
                  null,
                  2
        ...
        try {
          ...
          await generateAlert(
            ...
            "service_restart",
            "info",
            `Service ${service_name} was restarted via ${restartResult.method}`
          );
      ...
      case "get_troubleshooting_info": {
        const config = await loadHealthConfig();
        const service = config.services?.find(
          (s) => s.name === args.service_name || s.id === args.service_name
        );
      ...
      case "check_group": {
        ...
        for (const serviceName of groupServices) {
          const service = config.services?.find(
            (s) => s.name === serviceName || s.id === serviceName
          );
      ...
      case "reload_config": {
        try {
          ...
          const serviceNames = new Set(
            (config.services || []).map((s) => s.name || s.id).filter(Boolean)
          );
      ...
      case "get_health_score": {
        ...
        if (yomiApi && (yomiApi.score === 0 || yomiApi.status === "degraded")) {
          recommendations.push({
            ...
            severity: "high",
            message:
              "Yomi API is not healthy. If tony-dell cannot be recovered, fail over to tony-omen.",
        ...
        if (postgres && (postgres.score === 0 || postgres.status === "degraded")) {
          recommendations.push({
            ...
            severity: "critical",
            message:
              "PostgreSQL is not healthy. Yomi depends on it. Review backup age and consider failover.",
            script: "/home/tony/.local/bin/yomi-failover-to-omen.sh",
            ssot: "docs/ssot/infrastructure/ssot.yomi-failover.yml",
          });
        }
