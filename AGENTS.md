...
- Key ops: `scripts/ada-key-url.sh <tony|michael|ada-pi> [--qr]` prints the unlock deep-link (`--qr` mints a **one-time** `/redeem/{token}` URL via `POST /api/auth/redeem-token` — burns on first GET, expires in `ADA_REDEEM_TTL_S`=600s, hands the device the key + session cookie); `scripts/rotate-ada-key.sh <instance>` rotates + restarts + prints the new link.
