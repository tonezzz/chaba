...
class Hindsight01Strategy extends Strategy {
  constructor(
    parameters = { buy_points: [], sell_points: [], auto_detect: true, min_change_pct: 0.01 }
  ) {
  ...
  autoDetectPoints() {
    ...
    console.log(
      `Processing ${this.data.length} data points for Hindsight-01 (peak/valley detection with future knowledge, min change: ${(this.minChangePct * 100).toFixed(1)}%)...`
    );
    ...
    console.log(
      `Hindsight-01: ${this.buyPoints.length} buy points, ${this.sellPoints.length} sell points (peak/valley detection with future knowledge)`
    );
  }
...
class Hindsight02Strategy extends Strategy {
  ...
  autoDetectPoints() {
    ...
    console.log(
      `Processing ${this.data.length} data points for Hindsight-02 (sensitive detection)...`
    );
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      const trendChanged =
        trendDirection !== null && currentTrend !== null && currentTrend !== trendDirection;
    ...
    console.log(
      `Hindsight-02: ${this.buyPoints.length} buy points, ${this.sellPoints.length} sell points`
    );
  }
