...
class ChartLoader {
  constructor(options = {}) {
    this.config = {
      ...
      timeframe:
        options.timeframe || localStorage.getItem("trade-canvas-selected-timeframe") || "1Y",
      chartType:
        options.chartType || localStorage.getItem("trade-canvas-chart-type") || "candlestick",
  ...
  updateSymbol(symbol) {
    ...
    try {
      localStorage.setItem("trade-canvas-selected-currency", symbol);
    } catch (e) {}
    this.loadData();
  }
  ...
  updateTimeframe(timeframe) {
    ...
    try {
      localStorage.setItem("trade-canvas-selected-timeframe", timeframe);
    } catch (e) {}
    this.loadData();
  }
