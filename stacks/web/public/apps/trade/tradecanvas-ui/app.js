...
class TradeCanvasApp {
  ...
  generateSampleData() {
    ...
    const basePrice =
      this.currentSymbol === "THB"
        ? 35.5
        : this.currentSymbol === "JPY"
          ? 150.0
          : this.currentSymbol === "EUR"
            ? 1.1
            : this.currentSymbol === "GBP"
              ? 1.3
              : this.currentSymbol === "DXY"
                ? 105.0
                : this.currentSymbol === "OIL"
                  ? 75.0
                  : 35.5;
  ...
  updateChart() {
    ...
    const candlestickData = this.data.map((item) => {
      const priceField =
        this.currentSymbol === "DXY" ? "value" : this.currentSymbol === "OIL" ? "price" : "rate";
  ...
  addIndicator(indicator) {
    ...
    const closePrices = this.data.map(
      (item) => item.close || item.rate || item.value || item.price
    );
  ...
  updateStatistics() {
    ...
    document.getElementById("stat-change").className =
      `stat-value ${change >= 0 ? "positive" : "negative"}`;
    ...
    if (this.activeIndicators.has("sma")) {
      const sma = this.calculateSMA(
        this.data.map((d) => d.close || d.rate || d.value || d.price),
        20
      );
  ...
  updateCrosshairInfo(data, time) {
    ...
    console.log(
      `Crosshair: ${date} - Open: ${data.open}, High: ${data.high}, Low: ${data.low}, Close: ${data.close}`
    );
  }
  ...
  updateChartTitle() {
    ...
    document.getElementById("chart-title").textContent =
      `${symbolDisplay} - ${this.currentTimeframe}`;
