...
class StrategyPanel {
  ...
  init() {
    ...
    window.runAllWithFeedback = () => {
      ...
      try {
        if (status)
          status.innerHTML =
            '<p style="color:#a8d5ff; font-size:11px;">Running all strategies...</p>';
        if (typeof window.runAllBacktest !== "function") {
          if (status)
            status.innerHTML =
              '<p style="color:#da3633; font-size:11px;">Run All not ready yet.</p>';
          return;
        }
        window.runAllBacktest();
      } catch (e) {
        if (status)
          status.innerHTML = '<p style="color:#da3633; font-size:11px;">' + e.message + "</p>";
        console.error(e);
      }
    };
    ...
    this.resultContainer.innerHTML =
      '<div id="single-result" style="margin-bottom:4px;"><p style="color:#a8d5ff; font-size:11px; margin-top:2px;">Click "Run Backtest" or "Run All" to see results.</p></div><div id="compare-table"></div>';
  ...
  buildControls() {
    ...
    html +=
      '<label style="font-size:11px; color:#a8d5ff; display:block; margin-bottom:2px;">Strategy:</label>';
    html +=
      '<select id="strategy-select" class="selector" style="width:100%; padding:4px; font-size:12px; background:#21262d; border:1px solid #30363d; border-radius:3px; color:#e6edf3;">';
    ...
    html +=
      '<button id="run-backtest" class="btn" style="width:100%; margin-top:6px; padding:8px; font-size:12px;">Run Backtest</button>';
    html +=
      '<button id="run-walkforward" class="btn" style="width:100%; margin-top:4px; padding:8px; font-size:12px; background:#8957e5; border-color:#8957e5;">Walk-Forward Analysis</button>';
    html +=
      '<button id="run-all" class="btn" style="width:100%; margin-top:4px; padding:8px; font-size:12px;">Run All & Compare</button>';
    ...
    document
      .getElementById("run-walkforward")
      .addEventListener("click", () => this.runWalkForward());
  ...
  runBacktest() {
    ...
    const settings = {
      capital:
        parseFloat(document.getElementById("strategy-capital").value) ||
        this.config.execution.initial_capital,
      ...
      tpPct: parseFloat(document.getElementById("strategy-tp").value) || 0.1,
  ...
  computeMetrics(result, settings) {
    ...
    const grossLosses = Math.abs(
      trades.filter((t) => t.profit < 0).reduce((a, t) => a + t.profit, 0)
    );
  ...
  runAll() {
    try {
      ...
      this.renderComparisonTable();
      if (
        this.config.display.show_chart_markers &&
        this.chartLoader.candlestickSeries &&
        this.backtests.length
      ) {
        this.setMarkers(this.backtests[0].result.trades);
      }
    } catch (e) {
      console.error("Run All error:", e);
      this.singleResult.innerHTML =
        '<p style="color:#da3633;">Run All failed: ' + e.message + "</p>";
    }
  }
  ...
  renderComparisonTable() {
    ...
    let html =
      '<div style="margin:4px 0; padding:6px; background:#21262d; border-radius:4px; border:1px solid #30363d;">';
    html +=
      '<div style="overflow-x:auto;"><table style="width:100%; font-size:10px; border-collapse:collapse;">';
    html +=
      '<thead><tr style="text-align:left; border-bottom:1px solid #30363d;"><th style="padding:2px 3px;">Strategy</th>';
    for (const key of order) {
  ...
  runWalkForwardAnalysis(key, params, settings) {
    ...
    if (totalData < trainingSize + testingSize) {
      this.singleResult.innerHTML =
        '<p style="color:#da3633;">Not enough data for walk-forward analysis. Need at least ' +
    ...
    this.singleResult.innerHTML =
      '<p style="color:#a8d5ff; font-size:11px;">Running walk-forward analysis...</p>';
  ...
  runStrategyOnData(key, params, data, settings) {
    ...
    if (inPosition) {
      const finalPrice =
        data[data.length - 1].close * (1 - settings.commission - settings.slippage);
  ...
  calculateSharpeRatio(result) {
    ...
    const returns = result.trades
      ...
      .map(
        (t) =>
          t.profit /
          (result.trades.find((b) => b.index < t.index && b.type === "buy")?.price * t.qty || 1)
      );
  ...
  displayWalkForwardResults(results, aggregated) {
    ...
    html +=
      '<tr style="background:#30363d;"><th style="padding:4px; text-align:left;">Period</th><th style="padding:4px; text-align:right;">Return</th><th style="padding:4px; text-align:right;">Win Rate</th><th style="padding:4px; text-align:right;">Trades</th></tr>';
...
function initComparePanel(chartLoader) {
  ...
  const fallBack = () => {
    document.getElementById("strategy-controls").innerHTML =
  ...
  loadPageConfig()
    .then((config) => {
      ...
      if (!config || !config.strategy_panel)
        throw new Error("strategy_panel not found in resolved UI config");
