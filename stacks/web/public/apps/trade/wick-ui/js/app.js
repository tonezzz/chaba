...
class TradingDashboard {
  ...
  async loadDollarIndex() {
    try {
      const response = await fetch(`${API_BASE_URL}/dollar_index?period=${this.currentPeriod}`);
      const data = await response.json();
      ...
      const prevPrice =
        tradeData.length > 1
          ? tradeData[tradeData.indexOf(trade) - 1]?.close ||
            ...
            tradeData[tradeData.indexOf(trade) - 1]?.value
          : price;
  ...
  showError(message) {
    ...
    document
      ...
      .insertBefore(errorDiv, document.querySelector(".dashboard-grid"));
