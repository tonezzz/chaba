(function () {
  ...
  function layout(data) {
    ...
    if (
      layoutCfg.auto !== false &&
      components.some((c) => typeof c.x !== "number" || typeof c.y !== "number")
    ) {
  ...
  function nodePort(c, side) {
    ...
    return side === "out" ? { x: c.x + halfW, y: c.y } : { x: c.x - halfW, y: c.y };
  }
  ...
  function select(id) {
    ...
    if (item.type)
      html += `<div class="sys-kv"><span>Type</span><span>${escapeHtml(item.type)}</span></div>`;
    html += `<div class="sys-kv"><span>Status</span><span>${escapeHtml(statusName)}</span></div>`;
    if (health && health.code)
      html += `<div class="sys-kv"><span>HTTP</span><span>${health.code}</span></div>`;
    if (health && health.error)
      html += `<div class="sys-kv"><span>Error</span><span>${escapeHtml(health.error)}</span></div>`;
    if (item.detail) {
