...
window.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") updateTabStatus();
});
updateTabStatus();
...
function getCookiesForUrl(url) {
  if (!chrome.cookies) {
    throw new Error(
      "chrome.cookies is not available. Please reload the AI Hub extension from chrome://extensions."
    );
  }
...
copyCookiesBtn.addEventListener("click", () => {
  ...
  navigator.clipboard
    .writeText(lastCookiesJson)
    .then(() => (cookieStatus.textContent = "Copied to clipboard."));
});
...
nbUploadBtn.addEventListener("click", async () => {
  ...
  if (!nbApiKey || !nbBaseUrl || !notebookId)
    return (nbStatus.textContent = "Set API key, base URL and notebook ID.");
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  ...
  try {
    ...
    nbStatus.textContent = res.ok
      ? `Uploaded: ${res.status} ${out}`
      : `Upload failed ${res.status}: ${out}`;
  } catch (err) {
...
function captureDebugInfo(site) {
  ...
  const result = {
    ...
    prompt: {
      ...
      rect: rectObj(el ? el.getBoundingClientRect() : null),
    },
    send: {
      found: !!btn,
      selector: getCssPath(btn),
      html: btn ? btn.outerHTML.slice(0, 500) : "",
      rect: rectObj(btn ? btn.getBoundingClientRect() : null),
    },
  };
  if (!adapter) {
    ...
    result.candidates = {
      ...
      buttons: Array.from(
        document.querySelectorAll(
          'button[aria-label], button[type="submit"], [role="button"][aria-label]'
        )
      )
        .slice(0, 8)
        .map(describe),
...
dumpPageBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const text = await res.text();
    debugStatus.textContent = res.ok
      ? `Dumped ${page.text.length} chars: ${text}`
      : `Dump failed: ${res.status} ${text}`;
  } catch (err) {
