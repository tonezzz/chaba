...
const adapters = {
  ...
  "aistudio.google.com": {
    ...
    sendSelector:
      'button[aria-label*="Run" i], button[title*="Run" i], button[data-testid="run-button"], .run-button',
  },
};
...
function readLastAssistant() {
  ...
  if (site === "claude.ai") {
    const nodes = document.querySelectorAll(
      '[data-testid="user-message"], .claude-message, .font-claude-message'
    );
    return nodes[nodes.length - 1]?.innerText || "";
  }
  if (site === "aistudio.google.com") {
    const turns = document.querySelectorAll(
      'ms-chat-turn[data-turn-role="Model"], ms-chat-turn, .chat-turn-container'
    );
