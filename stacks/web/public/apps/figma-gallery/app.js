...
function render(captures) {
  ...
  captures.forEach((item) => {
    ...
    card.className =
      "bg-card rounded-xl border border-gray-800 overflow-hidden hover:border-cyan-500 transition cursor-pointer group";
    const tags = (item.tags || [])
      .map((tag) => `<span class="px-2 py-1 rounded text-xs ${tagColor(tag)}">${tag}</span>`)
      .join("");
    card.innerHTML = `
