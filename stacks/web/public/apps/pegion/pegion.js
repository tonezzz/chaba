...
function renderHero(data) {
  ...
  const countries = data.stats?.countries ?? new Set(Object.keys(data.participants || {})).size;
  hero.innerHTML = `
...
function renderParticipants(data) {
  ...
  const entries = Object.entries(data.participants || {}).sort((a, b) => b[1] - a[1]);
  if (!entries.length) {
    el.innerHTML = '<p class="pg-muted">No data.</p>';
    return;
  }
  const max = entries[0][1];
  el.innerHTML = `<div class="pg-bar-list">${entries
    .map(
      ([code, count]) => `
...
function renderSponsors(data) {
  ...
  el.innerHTML = (data.sponsors || [])
    .map((s) => `<span class="pg-sponsor">${escapeHtml(s)}</span>`)
    .join("");
}
...
function renderContact(data) {
  ...
  const phones = (c.phones || [])
    .map((p) => `<a href="tel:${escapeHtml(p.replace(/[^+0-9]/g, ""))}">${escapeHtml(p)}</a>`)
    .join(" · ");
  el.innerHTML = `
