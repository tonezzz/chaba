...
async function loadMessagesForDate(chatId, dateStr) {
  ...
  if (sortedMessages.length > 0) {
    ...
    const lastTimestamp =
      sortedMessages[sortedMessages.length - 1].deliveredTime ||
      sortedMessages[sortedMessages.length - 1].createdTime;
    ...
    console.log(
      "messages.js: Last message deliveredTime:",
      sortedMessages[sortedMessages.length - 1].deliveredTime
    );
    console.log(
      "messages.js: Last message createdTime:",
      sortedMessages[sortedMessages.length - 1].createdTime
    );
...
function renderMessage(message) {
  ...
  let content = "";
  const isMediaOnly =
    !text || text.trim() === "" || (text.startsWith("{") && text.includes("keyMaterial"));
...
async function renderMessagesForDate(chatId, dateStr) {
  console.log(
    "messages.js: renderMessagesForDate called with chatId:",
    chatId,
    "dateStr:",
    dateStr
  );
  ...
  try {
    ...
    if (messages.length > 0) {
      ...
      console.log(
        "messages.js: First message Thailand time:",
        DateUtils.msToIso(messages[0].timestamp)
      );
    }
    ...
    if (batchBtn) {
      const hasUnanalyzedMedia = messages.some(
        (m) =>
          m.mediaType &&
          ["image", "photo", "sticker"].includes(m.mediaType.toLowerCase()) &&
...
async function analyzeBatchMedia(btn) {
  ...
  const confirmed = confirm(
    `Analyze ${unanalyzedMedia.length} media items? This will use API quota.`
  );
  if (!confirmed) return;
      ...
      if (job.status === "completed") {
        ...
        toggleBtns.forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            analysisDiv.querySelector(".final-view").style.display =
              view === "final" ? "block" : "none";
            analysisDiv.querySelector(".full-view").style.display =
              view === "full" ? "block" : "none";
          });
        });
