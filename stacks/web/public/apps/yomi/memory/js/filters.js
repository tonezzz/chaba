...
function handleFilterChange(event) {
  ...
  if (filterType === "conversation") {
    if (isChecked) {
      ...
      window.memoryState.filters.conversations = window.memoryState.filters.conversations.filter(
        (v) => v !== value
      );
    }
    ...
    if (isChecked) {
      ...
      window.memoryState.filters.topics = window.memoryState.filters.topics.filter(
        (t) => t !== value
      );
    }
  }
...
function applyFilters() {
  const { search, startDate, endDate, conversations, topics, memoryTypes } =
    window.memoryState.filters;
...
function updateStats(memories) {
  ...
  window.memoryState.stats.totalConversations = totalConversationsEl
    ? new Set(memories.flatMap((m) => m.sources || [])).size
    : 0;
}
