...
function setupEventListeners() {
  ...
  // Memory type filters
  const memoryTypeCheckboxes = document.querySelectorAll(
    '.filter-section:last-child input[type="checkbox"]'
  );
  memoryTypeCheckboxes.forEach((checkbox) => {
    checkbox.addEventListener("change", (e) => {
      ...
      if (isChecked) {
        ...
        window.memoryState.filters.memoryTypes = window.memoryState.filters.memoryTypes.filter(
          (t) => t !== value
        );
      }
