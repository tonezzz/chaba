...
function analyzeTopicEvolution(memories) {
  ...
  const emergingTopics = Object.keys(secondTopics).filter(
    (topic) => !firstTopics[topic] || secondTopics[topic] > firstTopics[topic] * 1.5
  );

  // Find declining topics (decreased in second half)
  const decliningTopics = Object.keys(firstTopics).filter(
    (topic) => secondTopics[topic] && firstTopics[topic] > secondTopics[topic] * 1.5
  );
...
function getTrendingTopics(memories) {
  ...
  return sortedTopics
    .map(
      ([topic, count]) => `<span class="topic-tag">${UiUtils.escapeHtml(topic)} (${count})</span>`
    )
    .join("");
}
