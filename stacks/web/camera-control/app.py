#!/usr/bin/env python3
"""Lightweight web control panel for the camera registry."""
import shutil
import subprocess
import sys
import threading
...
FRIGATE_DIR = REPO_ROOT / "stacks" / "nvr"
...
# Set True to copy stacks/nvr/generated/camera-map.html -> stacks/web/public/camera-map.html after edits.
SYNC_WEB_MAP = False
...
def _regenerate_files(registry):
    ...
    if SYNC_WEB_MAP:
        src = FRIGATE_DIR / "generated" / "camera-map.html"
...
def api_sync():
    ...
    dst = REPO_ROOT / "stacks" / "web" / "public" / "camera-map.html"
    if not src.exists():
        return jsonify({"error": "stacks/nvr/generated/camera-map.html not found"}), 404
...
@app.route("/map")
def map_page():
    return send_file(FRIGATE_DIR / "generated" / "camera-map.html")
