...
CONFIG_YML = SCRIPT_DIR / "generated" / "config.yml"
MAP_HTML = SCRIPT_DIR / "generated" / "camera-map.html"
...
DEFAULT_RECORD_RETAIN = {"alerts_days": 1, "detections_days": 1, "continuous_days": 0, "motion_days": 0}
...
def generate_map_html(registry):
    ...
    for cam in cameras:
        ...
        location_str = json.dumps(location, ensure_ascii=False) if location else "null"
        view = cam.get("view")
        view_str = json.dumps(view, ensure_ascii=False) if view else "null"
        description = cam.get("description")
        description_str = json.dumps(description, ensure_ascii=False) if description else "null"
        perspective = cam.get("perspective")
        perspective_str = json.dumps(perspective, ensure_ascii=False) if perspective else "null"
        cam_lines.append(
            f'            {{\n'
    ...
    # Snapshot images are proxied through Caddy on port 8080 so viewers only need access to :8080
    html = html.replace("http://localhost:5000/api/", "/frigate/api/")
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\n💡 Restart Frigate: docker compose -f stacks/nvr/docker-compose.yml restart")
        return
